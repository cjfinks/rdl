...
%\title{Stability in Sparse Coding}
\title{Sparse coding is well-posed for diverse enough samples}
...
% ALTERNATIVELY...define the problem as everyone does, as matrix recovery, and then say these ambiguities come from working in the wrong space. Go to that space. 
% Also I should translate epsilon into angle, i.e. the norm in the matrix space into the metric in the space of subspaces.
% Will this allow me to easily generalize to Grassmannians of higher dimensionality?
% Will the projective space screw up the fact that the coeffs of a are in [0,1] (since lines identify a vector and its negative)

%For instance, the dataset $Y$ may have been thought to arise as a sequence of linear measurements:
%\[ \mathbf{y} = A\mathbf{a} + \mathbf{n} \]
%equivalence class defined by scalings
%for some matrix $A \in \mathbb{R}^{n \times m}$, vectors $\mathbf{a} \in \mathbb{R}^m$ having no more than $k$ nonzero elements ($k$-sparse), and bounded noise $|\mathbf{n}| \leq \varepsilon$, and we may wish to identify the set of columns of $A$ (i.e. the dictionary) to within some error commensurate with $\varepsilon$.
\section{Introduction}
%Many applications of signal processing have emerged from the ability to...

\IEEEPARstart{S}{parse} coding or dictionary learning algorithms learn to represent each signal in a set $Y = \{\mathbf{y}_1, \ldots, \mathbf{y}_N\} \subset \mathbb{R}^n$ as a linear combination of few vectors drawn from an inferred \emph{dictionary} $\mathcal{D} \subset \mathbb{R}^n$, where $n \leq |\mathcal{D}| \ll N$ \cite{?; see \cite{} for a comphrehensive review of techniques}. One of the domains of application of these algorithms has been in solving inverse problems, wherein the dictionary elements and reconstruction coefficients are interpreted as real physical parameters to be identified from the data \cite{}. A proper interpretation of these results demands, however, a characterization of the set of degenerate solutions and their stability with respect to noise. 

In this note, we provide conditions guaranteeing uniqueness and stability of solutions to the sparse coding problem. The term `uniqueness' demands clarification in this context, however, since given any dictionary we can generate a distinct but equally expressive (through linear combination of its elements) dictionary by scaling its elements by some non-zero factors. One can therefore take `uniqueness' to mean degenerate only up to this ambiguity intrinsic to the statement of the problem. Alternatively, rather than discuss the uniqueness of dictionaries whose elements are vectors combined linearly to reconstruct the data, we can speak of sets of lines through the origin which are combined via the subspace sum $U+V := \{w: w = u + v, u \in U, v \in V\}$. These sets of lines belong to the projective space $P^{n-1}(\mathbb{R})$ defined by identifying all points in $\mathbb{R}^{n} \setminus \{0\}$ which are non-zero scalings of one another. 

\begin{definition}
Fix a dataset $Y = \{\mathbf{y}_1, \ldots, \mathbf{y}_N\} \subset \mathbb{R}^n$. A \textbf{$(m,k)$-scaffolding} of $Y$ is a set $\mathcal{U} = \{U_1, \ldots, U_m\}$ of lines through the origin which satisfies for all $i = 1, \ldots, N$, 
\[ \mathbf{y}_i \in U_{j_1} + \ldots + U_{j_k} \]
%
for some set of indices $\{j_1, \ldots, j_k\}$ on which we say $\mathbf{y}_i$ is \textbf{supported}.
\end{definition}
%scaffolding is punny because it provides support.

\begin{question}\label{Q1}
When does $Y$ have a unique $(m,k)$-scaffolding? 
\end{question}
\begin{question}
Given a $(m,k)$-scaffolding for $Y$, when does each datum in $Y$ have a unique support in this scaffolding?
\end{question}

This question was first addressed in the theoretical works \cite{} which determined how many samples $N$ in the dataset $Y$ are sufficient to guarantee uniqueness. We extend these analyses by asking what happens if the data are noisy and therefore do not necessarily all lie in the subspace sum but rather within some $\varepsilon$-ball around it. To address this question of \emph{stability}, we introduce the following metric on finite subsets of $P^{n-1}(\mathbb{R})$. Let $\mathcal{U} = \{U_i\}_{i \in \mathcal{I}}$ and $\mathcal{V} = \{V_i\}_{i \in \mathcal{I}}$ both be sets of lines indexed by $\mathcal{I}$. We choose:
% maybe i could change this to allow for the subsets to be of different sizes
\[ d(\mathcal{U}, \mathcal{V}) = \min_{\sigma \in \frak{S}_{|\mathcal{I}|}} \max_{i \in \mathcal{I}} d'(U_i, V_{\sigma(i)}) \]
%
where $\frak{S}_m$ is the set of permutations on $m$ elements and $d'$ is any metric on $P^{n-1}(\mathbb{R})$ (we will specify one later). [Prove this is a metric. Min-Max Theorem and Compactness?] This measures the largest distance (as given by $d'$) between corresponding lines in the two sets, when the lines have been put into a one-to-one correspondence yielding the smallest possible such value. Let $\mathcal{B}_\varepsilon(\mathbf{X})$ denote the $\varepsilon$-neighbourhood of the set $X$.

\begin{definition}\label{Uniqueness}
We say a $(m,k)$-scaffolding of a dataset is \textbf{stable} if for every $\delta \geq 0$ there exists a nonnegative $\varepsilon = \varepsilon(\delta)$ (with $\varepsilon > 0$ when  $\delta > 0$) such that all set of lines $\mathcal{V} = \{V_1, \ldots, V_m\}$ satisfying
\[ \mathbf{y}_i \in \mathcal{B}_\varepsilon( V_{j_1} + \ldots + V_{j_k} ) \]
for some set of indices $\{j_1, \ldots, j_k\}$ satisfy:
\begin{align}\label{def1}
d(\mathcal{U}, \mathcal{V}) \leq \delta.
\end{align}
\end{definition}

\begin{question}
When does $Y$ have a stable $(m,k)$-scaffolding? 
\end{question}
[ When is the support stable? ]

It is easy to see how a solution to Problem \ref{DUTproblem} directly informs the interpretation of any solution to Problem \ref{P1}. Suppose a set of vectors has a robustly identifiable $k$-sparse representation in $\mathbb{R}^m$ and that $Y$ is a sequence of measurements, each sampled from an $\varepsilon$-ball drawn around these vectors, where $\varepsilon(\delta)$ is defined as in Def.~\ref{Uniqueness} for some fixed $\delta \geq 0$. Then by the triangle inequality, any solution to Problem \ref{P1} necessarily satisfies \eqref{def1}. That is, the solution to \eqref{InverseProblem} is uniquely determined up to an error commensurate with measurement noise.


The main finding of this work is that so long as no two $k$-element subsets of dictionary elements span the same $k$-dimensional subspace, then the $k$-sparse representation of $Y$ in Problem \ref{DUTproblem} is unique and stable provided the data are sufficiently diverse. [This is equivalent to the spark condition.] Note that this condition is necessary if we wish for the unique decomposition to be a \emph{direct} sum of subspaces, in particular. 
...
Given the support set for its $k$ nonzero entries, a \textbf{random draw} of $\mathbf{a}$ is the $k$-sparse vector with support entries chosen uniformly from the interval $[0, 1] \subset \mathbb{R}$, independently. 
\end{definition}
...
|A_i - (BPD)_i|_2 = |A_i - \frac{c'_i}{c_i}B_{\pi(i)}|_2 \leq \frac{\varepsilon}{|c_i|} \leq C\varepsilon \ \ \text{for all } i \in [m].
\end{align}
