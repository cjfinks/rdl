...
\title{Uniqueness in sparse coding}
...
\begin{abstract}
Sparse coding or sparse dictionary learning has exposed underlying structure in many kinds of natural data. Here, we generalize to the case of measurement noise previous results guaranteeing when the learned dictionary and sparse codes are unique up to inherent permutation and scaling ambiguities.  Central to our proofs is a useful lemma in combinatorial matrix theory allowing us to derive bounds on the number of samples necessary to guarantee uniqueness. We also provide probabilistic extensions to our robust identifiability theorem and an extension to the case when only an upper bound on the number of dictionary elements is known a priori. Our results inform the interpretation of sparse structure learned from data: whenever the conditions to our theorems are met, any sparsity-constrained algorithm that succeeds in approximately reconstructing the data recovers the original dictionary and sparse codes up to an error commensurate with the noise. Applications are given to smoothed analysis, communication theory, and engineering.
\end{abstract}
...
\IEEEPARstart{S}{parse} coding or sparse dictionary learning algorithms learn to represent each signal in a set $Y = \{\mathbf{y}_1, \ldots, \mathbf{y}_N\} \subset \mathbb{R}^n$ as a linear combination of few vectors drawn from an inferred \emph{dictionary} $\mathcal{D} \subset \mathbb{R}^n$, where $n \leq |\mathcal{D}| \ll N$ \cite{?; see \cite{} for a comphrehensive review of techniques}. One application domain for these algorithms has been solving inverse problems, wherein the dictionary elements and reconstruction coefficients are interpreted as real physical parameters to be identified from data. These results cannot be properly interpreted, however, without a characterization of the set of equivalent solutions and their stability with respect to noise. In this note, we provide conditions guaranteeing stable uniqueness of solutions to the sparse coding problem.

Suppose that for some $A \in \mathbb{R}^{n \times m}$ we can model $Y$ as a sequence of linear measurements:
\begin{align}\label{LinearModel}
\mathbf{y} = A\mathbf{a} + \mathbf{n},
\end{align}
%
in which the underlying \emph{sparse code} $\mathbf{a} \in \mathbb{R}^{m}$ has at most $k$ nonzero entries (\textit{$k$-sparse}) and the \emph{noise} vector $\mathbf{n}$ is bounded, $|\mathbf{n}|_2 \leq \varepsilon$. This model has an associated inverse problem.  % \cite{} [*** latest review of sparse coding algorithms ****]. 
...
Phrasing the problem in this way necessitates some clarification of the term ``uniqueness", however, since \eqref{InverseProblem} admits many solutions, even in the noiseless case ($\varepsilon = 0$). Let $Y$ and $X$ be matrices with columns $\mathbf{y}_i$ and $\mathbf{a}_i$, respectively, so that $Y = AX$ for $X$ with $k$-sparse columns. Then we also have $Y = BX'$ for $X'$ with $k$-sparse columns whenever $B = AD^{-1}P^{-1}$ and $X' = PDX$ with $P \in \mathbb{R}^{m \times m}$ a permutation matrix  and $D \in \mathbb{R}^{m \times m}$ an invertible diagonal. Any particular solution to \eqref{InverseProblem} thus generates many other solutions through actions $PD$, which together constitute a \emph{transformation group} \cite{BilinInv} associated to the problem. The question of when every solution $Y = BX'$ must necessarily be of this form (i.e., $X' = PDX$ and $B = AD^{-1}P^{-1}$ for some $P$, $D$) was addressed in the theoretical works \cite{Georgiev05, Aharon06, Hillar15} and determines whether $A$ and $X$ are \emph{identifiable} from $Y$ up to this inherent ambiguity. We note that the permutation ambiguity reflects column (the dictionary) symmetries, whereas the scaling ambiguity is inherent in the sparsity constraint.

Here, we make progress on the uniqueness problem in the presence of measurement uncertainty. To address this more general situation, we consider the following stability criterion.
...
\begin{problem}[Identifiability]\label{DUTproblem}
...
It is easy to see how a solution to Problem \ref{DUTproblem} directly informs the interpretation of one to \eqref{InverseProblem}. Suppose $Y$ are measurements noisily generated as in \eqref{LinearModel}, where the underlying dictionary $A$ and sparse codes $\mathbf{a}_i$ have a robustly identifiable $k$-sparse representation in $\mathbb{R}^m$. Fix $\delta_1, \delta_2 \geq 0$ and let $\varepsilon(\delta_1, \delta_2)$ be as in Def.~\ref{Uniqueness}. Since $|\mathbf{y}_i - A\mathbf{a}_i|_2 \leq \varepsilon$ for all $i$, if $\varepsilon \leq \frac{1}{2}\varepsilon(\delta_1, \delta_2)$ then, by the triangle inequality, any solution to \eqref{InverseProblem} necessarily satisfies \eqref{def1} for some $P$ and $D$. That is, the solution to \eqref{InverseProblem} is uniquely determined up to the inherent permutation-scaling ambiguity and an error commensurate with noise.
...
\end{align}
known in the sparse recovery literature as the \emph{spark condition}, then the $k$-sparse representation of $Y$ in Problem \ref{DUTproblem} is robustly identifiable provided the data are sufficiently diverse. Note that this condition is necessary given only that $\mathbf{a}_i$ are $k$-sparse \cite{gleichman2011}. 
%[ *** See Thm 2.8 or Cor 2.9 of identifiability of BIP paper? Can we cite this here? Can we then motivate the restrictions on X with these theorems too? ***]

Before stating this result more precisely, we explain how the spark condition relates to the \emph{lower bound} \cite{grcar2010matrix} of $A$, which is the largest number $\alpha$ such that $|A\mathbf{x}|_2 \geq \alpha|\mathbf{x}|_2$ for all $\mathbf{x}$. A straightforward compactness argument shows that every injective linear map has a nonzero lower bound; hence, if $A$ satisfies the spark condition then every submatrix formed from $2k$ of its columns has a nonzero lower bound. We therefore define the following domain-restricted lower bound of $A$:
\begin{align}
...
Recall that a \textit{cyclic order} on $[m] := \{1, \ldots,m\}$ is an arrangement of $[m]$ in a circular necklace, and that an \textit{interval} is a contiguous sequence of such elements.  Also, let $A_j$ denote the $j$th column of a matrix $A$.  The following is our main result.
...
[**** go ahead and do your best, citing people, and writing this sentence out for the adults that are your readers. -cjh  ****]

An important consequence of this result is that for sufficiently small reconstruction error, the original dictionary and $k$-sparse vectors are determined up to a commensurate error and permutation-scaling ambiguity. Recalling Def. \ref{Uniqueness}, for fixed $\delta_1, \delta_2 \geq 0$, we have that \eqref{y-Bb} implies \eqref{def1} for any $\varepsilon \in [0, \varepsilon_0)$ satisfying:
...
We state here the main implications of our probabilistic results. To keep exposition simple, our statements are based upon the following elementary construction of \emph{random sparse vectors} (although many ensembles suffice, e.g. \cite[Sec.~\S 4]{baraniuk2008simple}).
...
\end{definition}
[ *** We don't actually use the second sentence of the above def ...could cut it out.] 
