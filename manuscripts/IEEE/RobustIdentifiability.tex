...
Evidently, for any fixed $A$ an unknown $k$-sparse source $\mathbf{a}$ can only be robustly identified from measurements $\mathbf{y}$ of the form \eqref{LinearModel} if $A$ is injective on the set of $k$-sparse vectors; that is, $A$ must satisfy:
...
\end{align}
%
commonly known in the literature as the \emph{spark condition}. As it turns out, even if $A$ is unknown a priori this condition is enough to guarantee that both $A$ and the $\mathbf{a}_i$ are robustly identifiable from a sufficiently diverse set of samples $\mathbf{y}_i$.
