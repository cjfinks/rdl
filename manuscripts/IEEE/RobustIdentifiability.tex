...
[********** this proof still needs to be made slicker (also, i think that the variable names in this proof here could be made nicer...) ********]
%======= GRAPH THEORY LEMMA =======
...
We next argue that no other $t$ has $q \in \pi(S_t)$.  Suppose now that $t \notin J$ is such that $q \in \pi(S_t)$; then, we would have $q \in \pi(S_t) \cap \pi(S_{j_1}) \cap \cdots \cap \pi(S_{j_k})$ and \eqref{EmptyToEmpty} would imply that the intersection of every $k$-element subset of $\{S_t\} \cup \{S_j: j \in J\}$ is nonempty. This would only be possible if $\{t\} \cup J = [m]$, in which case the result then trivially holds since then $q \in \pi(S_i)$ for all $i \in [m]$.  Finally, letting $Q_{m-1} \subset Q_m$ be the set of elements of $Q_m$ not having $q$ as a first coordinate, we have $|Q_{m-1}| = (m-1)k$ and the result follows by iterating the above arguments [**** why? ****].  
\end{proof}
