...
Even when all $\mathbf{n}_i = 0$, however, there many solutions to \eqref{DictionaryLearning}. Let $Y$ and $X$ be the matrices with columns $\mathbf{y}_i$ and $\mathbf{a}_i$, respectively, so that $Y = AX$ for $X$ with $k$-sparse columns. Then by letting $\tilde A = AD^{-1}P^{-1}$ and $\tilde X = PDX$ for any permutation matrix $P \in \mathbb{R}^{m \times m}$ and invertible diagonal matrix $D \in \mathbb{R}^{m \times m}$, we also have $Y =\tilde A \tilde X$ for $\tilde X$ with $k$-sparse columns. The ``uniqueness" of any solution to \eqref{DictionaryLearning} should therefore only be considered up to transformation by matrices of the form $PD$, the set of which form the \emph{ambiguity transform group} \cite{?} associated to this particular form of bilinear inverse problem. 

Whether every solution $Y = \tilde A \tilde X$ must \emph{necessarily} be of the form $\tilde X = PDX$ and $\tilde A = AD^{-1}P^{-1}$ was the subject of the theoretical works \cite{Georgiev05, Aharon06, Hillar15}.  
