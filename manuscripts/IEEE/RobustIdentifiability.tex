...
\title{When is Sparse Matrix Factorization Well-Posed?}
...
\section{Implications for Dictionary Learning by Constrained Optimization}

In this section we apply our results to constrained optimization problems commonly used to solve \eqref{InverseProblem}. The first is:
...
\begin{problem}\label{DL0}
When are global solutions to \eqref{DictionaryLearning} unique up to noise and inherent permutation-scaling ambiguities?
\end{problem}

At first glance one might think that the same conditions guaranteeing the well-posedness of \eqref{InverseProblem} solve Problem \ref{DL0}, but it need not be the case that the solution to \eqref{DictionaryLearning} necessarily satisfies $|\mathbf{a}_i|_0 \leq k$ for all $i = 1, \ldots, N$. When is this the case? 

We also consider the following variation of the dictionary learning problem:
...
\begin{problem}\label{DL1}
When are global solutions to \eqref{DictionaryLearningL1} unique up to noise and inherent permutation-scaling ambiguities?
\end{problem}
