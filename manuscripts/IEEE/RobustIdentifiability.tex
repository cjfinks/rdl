...
Left unanswered by these works is the question of whether solutions to \eqref{DictionaryLearning} indeed converge to $A$ and $X$ as $\varepsilon \rightarrow 0$. To address this issue, we introduce the following robust extension to the notion of identifiability in sparse dictionary learning.
...
In short, a $k$-sparse representation of a dataset is robustly identifiable when the only other matrices and $k$-sparse vectors that can well-approximate it are necessarily close to some permutation and scaling of the originals, and are identical in the noiseless limit.
