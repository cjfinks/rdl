...
\IEEEPARstart{A}{} fundamental problem in data analysis is to compute representations of data that reveal intrinsic structure. In recent years, a plethora of algorithms (e.g., see \ref{} ***add punchiest recent algo refs***) have been developed for uncovering \emph{sparse} structure in datasets $Y = \{\mathbf{y}_1, \ldots, \mathbf{y}_N\} \in \mathbb{R}^n$ by approximately solving the following constrained optimization problem:
\begin{align}\label{DictionaryLearning}
\min_{B, \, |\mathbf{b}_i |_0 \leq k} \  \sum_{i=1}^N |\mathbf{y}_i - B\mathbf{b}_i|_2,
\end{align}
%
for unknown $B \in \mathbb{R}^{n \times m}$ and $\mathbf{b}_1, \ldots, \mathbf{b}_N \in \mathbb{R}^m$ with sparsity $k \ll m$ and latent dimensionality $m \ll N$. Known in the literature as ``sparse dictionary learning", ``sparse coding", or ``sparse matrix factorization", this approach emphasizes parsimony in representation by approximating each $\mathbf{y}_i$ as a linear combination of at most $k$ columns of some matrix $B$.  Typically, the optimization (\ref{DictionaryLearning}) is relaxed so that the $\ell_1$-norm $|\cdot|_1$ replaces the $\ell_0$-norm $|\cdot|_0$ (the number of nonzero entries in a vector), as the latter typically determines an NP-hard problem \cite{} [Tollman 2015???] whereas the former allows for tractable convex programs [***Do we actually know this?  what's the latest ref on how efficient SCA is with $\ell_1$-norm***] that generically have the same solutions.
