...
\begin{lemma}\label{NonEmptyLemma} Fix positive integers $k < m$, and let $T = \{S_1, \ldots, S_m\}$ be the set of length $k$ intervals in a cyclic order of $[m]$. Suppose there exists a map $\pi: T \to {[m] \choose k}$ such that
...
\begin{proof} Consider the set $Q_m = \{ (r,t) : t \in [m], r \in \pi(S_t) \}$, which has $mk$ elements. By the pigeon-hole principle, there is some $q \in [m]$ and $J \in {[m] \choose k}$ such that $(q, j) \in Q_m$ for all $j \in J$. In particular, we have $q \in \cap_{j \in J} \pi(S_j)$ so that from \eqref{NonEmpty} there must be some $p \in [m]$ with $p \in \cap_{j \in J} S_j$. Since $|J| = k$, this is only possible if the elements of $J = \{j_1, \ldots, j_k\}$ are consecutive modulo $m$ with $|\cap_{j \in J} S_j| = 1$. Hence $|\cap_{j \in J} \pi(S_j)| = 1$ as well.
...
By iterating the above arguments we arrive at a partitioning of $Q_m$ into sets $R_i = Q_i \setminus Q_{i-1}$ for $i = 1, \ldots, m$, each having a unique element of $[m]$ as a first coordinate common to all $k$ elements while having second coordinates which form a consecutive set modulo $m$. In fact, each set of $k$ consecutive integers modulo $m$ is the set of second coordinates of some $R_i$ since by \eqref{NonEmpty} there can be no two distinct sets $R_i$ with the same set of second coordinates.
\end{proof}
%==== PROOF OF MAIN LEMMA =======
