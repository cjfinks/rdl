...
\end{align}
As the dictionary learning problem \eqref{DictionaryLearning} is NP-hard in general \cite{razaviyayn2015computational, Tillmann15}, algorithms solving it implement strategies that lead to approximate solutions, typically by greedy methods or by convex relaxation of the $\ell_0$ pseudo-norm $|\cdot|_0$ (the number of nonzero entries in a vector) to the $\ell_1$-norm $|\cdot|_1$ when appropriate. For a recent comprehensive survey of such techniques, see the review paper \cite{Zhang15}.
...
One can easily deduce the following necessary condition on the matrix $A$ by setting $\delta_1 = \delta_2 = 0$ in \eqref{def1}:
...
[ *** Actually, this is kind of out of my ass. The $\mathbf{a}_i$ probably have to form a basis or something and even then I'm not sure it's that easy to deduce. I mean we only have a finite set of points to identify, so $A$ really need only be injective on those points, or maybe the proofs require injectivity over the supports of those points (i.e. the supports in some cyclic order of $[m]$). Should we introduce the spark condition without explaining why it's necessary..? Or am I over-thinking this? *** ]

This condition is commonly known in the literature as the \emph{spark condition} and states that $A$ is injective on the set of $k$-sparse vectors. Our main result states that this condition is enough to guarantee that both $A$ and the $\mathbf{a}_i$ are robustly identifiable from a sufficiently diverse set of samples $\mathbf{y}_i$.
...
We now state the main implications of our probabilistic results. To keep our exposition simple, our probabilistic statements are based upon the following elementary construction of \emph{random sparse vectors} (although many ensembles will suffice, e.g. \cite[Sec.~\S 4]{baraniuk2008simple}).
...
[ *** We stated this corollary in the intro. I don't suppose we need to state it again...? ***]
\begin{corollary}
