...
We note that a $k$-sparse representation is $C$-stable precisely when the maximum possible value of $\|(A-BPD)\mathbf{e}_i\|_2$ as a function of $\varepsilon$ is Lipschitz continuous in some neighbourhood of $0$ for $i = 1, \ldots, m$. We show later that $C$-stability implies robust identifiability. Specifically, we show that if the representation is $C$-stable then there exists a positive $\varepsilon_0$ such that
...
Note how by the comments preceding the statement of the problem, $Y$ has a unique $k$-sparse representation in $\mathbb{R}^m$ whenever $\{A\mathbf{a}_1, \ldots, A\mathbf{a}_N\}$ has a $k$-sparse representation in $\mathbb{R}^m$ that is $C$-stable over $[0, 2\max_i \|\mathbf{n}_i\|)$.
...
The stability constant $C$ can be calculated as follows. Letting $X  = (\mathbf{a}_1 \cdots \mathbf{a}_N) \in \mathbb{R}^{n \times N}$ and $J(S) = \{j : \text{supp}(\mathbf{a}_j) = S\}$, define:
\begin{align}\label{epsilon0}
\varepsilon_1 = \frac{ \phi_k(A) \ell_2(A) }{\sqrt{2k^3} } \min_{S \in T} \ell_k(AX_{J(S)}).
\end{align}
If $A$ has unit norm columns then the representation is $C$-stable over $[0, \varepsilon_1)$, where
\begin{align}\label{Cdef}
C = \frac{\ell_2(A)}{\sqrt{2} \varepsilon_1}, 
\end{align}
%
and the positive constant referred to in \eqref{b-PDa} is $\varepsilon_0 = C^{-1}\ell_{2k}(A)/\sqrt{2k}$.
\end{remark}
...
% ======== b - PDa =========
We note that from this result we can, in general, derive an upper bound on $\|\mathbf{b}_i - PD\mathbf{a}_i\|_2$ as follows. For small enough $\varepsilon$ we have for all $2k$-sparse $\mathbf{x} \in \mathbb{R}^m$:
\begin{align*}
...
\end{align*}
%
which proves \eqref{b-PDa} for $\varepsilon_0 = C^{-1}\ell_{2k}(A)/\sqrt{2k}$.
...
$\|(A - BPD)\mathbf{e}_i\|_2 \leq C\varepsilon$ for $C = \frac{\ell_2(A)}{\varepsilon_1\sqrt{2}}$.
\end{proof}
...
\begin{corollary}
Suppose $m, n$, and $k$ obey inequality \eqref{CScondition}.  If $N = mk{m \choose k}$ randomly drawn vectors $\mathbf{a}_1, \ldots, \mathbf{a}_N \in \mathbb{R}^m$ are such that for every interval of length $k$ in some cyclic ordering of $\{1, \ldots, m\}$ there are $k{m \choose k}$ vectors $\mathbf{a}_i$ supported on that interval then, with probability one, almost every matrix $A \in \mathbb{R}^{n \times m}$ gives $\{A\mathbf{a}_1, \ldots , A\mathbf{a}_N \}$ a $C$-stable $k$-sparse representation in $\mathbb{R}^m$.
\end{corollary}
