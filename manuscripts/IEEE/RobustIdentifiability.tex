...
In short, a $k$-sparse representation of a dataset is robustly identifiable when the only other matrices and $k$-sparse vectors that can well-approximate it are necessarily close to some permutation and scaling of the originals.

To see how this definition directly applies to approximately sparse datasets, fix $A \in \mathbb{R}^{n \times m}$ and $k$-sparse $\mathbf{a}_1, \ldots, \mathbf{a}_N$ such that $\{A\mathbf{a}_1, \ldots, A\mathbf{a}_N\}$ is robustly identifiable. Let $\delta_1, \delta_2 > 0$ and suppose that $Y = \{\mathbf{y}_1, \ldots, \mathbf{y}_N\}$ satisfies $|\mathbf{y}_i - A\mathbf{a}_i|_2 \leq \varepsilon/2$ for $i = 1, \ldots, N$, where $\varepsilon$ is as in Def.~\ref{Uniqueness}. % for some $\varepsilon_1 \leq \varepsilon$. 
...
Before stating our solution to this problem, let us consider the following simpler version. Suppose we have some mixture $\mathbf{y}$ generated as in \eqref{LinearModel} by a \emph{known} matrix $A$. Can we identify the $k$-sparse source $\mathbf{a}$? Evidently, regardless of noise level, this is only possible if $A$ is injective on the set of $k$-sparse vectors; that is,
...
\end{align}
commonly known in the literature as the \emph{spark condition}. More detailed conditions guaranteeing the recovery of $\mathbf{a}$ in this case were first presented in \cite{candes2006stable}. But what about when $A$ is not known a priori? Our main contribution here is that given enough sufficiently diverse samples $\mathbf{y}_i$, the matrix $A$ itself can be estimated up to inherent permutation-scaling ambiguities and noise levels. If the noise level is low enough then this estimate also obeys the spark condition, which makes possible the approximate recovery of the $k$-sparse vectors $\mathbf{a}_i$, too. 
...
\end{align}
Furthermore, there exists some $\varepsilon_1 \leq \varepsilon_0$ such that if $\varepsilon < \varepsilon_1$ then $B$ satisfies the spark condition as well, and:
...
\end{align}
\end{theorem}
An important consequence of this result is that for sufficiently small reconstruction error, the original dictionary and sparse codes are determined up to a commensurate error, independent of alternate representation.  (In other words, we may choose $\varepsilon$ on the same order as $\delta_1$ or $\delta_2$ in Def.~\ref{Uniqueness}.) Formulae for  $C$ and $\varepsilon_0$ [ *** and $\varepsilon_0'$ ***] in terms of $A$ and the $\mathbf{a}_i$ are provided in \eqref{epsilon0} and \eqref{Cdef}, below. 
...
\end{align*}
%
where in the last inequality we may drop the absolute value since $\varepsilon < \varepsilon_1$. Hence, $\ell_{2k}(BPD) \geq \ell_{2k}(A) - C\varepsilon \sqrt{2k}$ and it follows that:
\begin{align*}
