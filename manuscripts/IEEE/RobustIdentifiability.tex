...
In the SCA framework, it is assumed that the model in \eqref{LinearModel} describes some real physical process and the goal is to infer the true underlying sparse codes $\mathbf{a}_i$ and dictionary $A$ from the data. While it is possible that the results of such inference may take on values idiosyncratic to the particular sparse coding algorithm being applied, our results suggest that uniqueness of this decomposition is the norm rather than the exception. In particular, the robust uniqueness proven here gives justification for declaring that learned features (e.g. the feature-tuned field potentials in \cite{agarwal2014spatially}) are indeed representative of intrinsic structure in the data. To this end, it would be useful to determine for general $k$ the best dependence of $\varepsilon$ on $\delta_1, \delta_2$ (see Def. \ref{Uniqueness}) possible as well as the minimum possible sample size $N$. We encourage the sparse coding community to extend our results and find a tight dependency of all the parameters, both for the sake of theory and practical applications.
...
C' = \left( \frac{ \sqrt{k^3}}{ \min(\phi_1(A), \phi_1(B)) } \right) \frac{\max_{j \in [m]} |A_j|_2}{\min_{i \in [m]} \ell_1(c_iA_i) }
= \sqrt{k^3} \left( \frac{\max_{j \in [m]} |A_j|_2}{\min_{i \in [m]}|c_iA_i|_2} \right)
\geq \max_{i \in [m]}|c_i|^{-1}, 
\end{align}
...
\end{align} 
Note that $c'_i \neq 0$ for all $i$ since then otherwise (by definition of $L_2(A)$) we reach the contradiction $|c_iA\mathbf{e}_i|_2 < \min_{i\in [m]}|c_iA\mathbf{e}_i|_2$. We will now show that $\pi$ is necessarily injective (and thus defines a permutation on $m$ elements). Suppose that $\pi(i) = \pi(j) = \ell$ for some $i \neq j$ and $\ell \in [m']$. Then, $|c_iA\mathbf{e}_i - c'_iB\mathbf{e}_{\ell}|_2  \leq \varepsilon$ and $|c_jA\mathbf{e}_j - c'_jB\mathbf{e}_{\ell}|_2 \leq \varepsilon$. Using the triangle inequality followed by application of the definition of $L$, we have:
\begin{align*}
(|c'_i| + |c'_j|) \varepsilon
...
\end{align*}
%
which contradicts \eqref{C'1} and our upper bound on $\varepsilon$. Hence, $\pi$ is injective. Letting $P = \left( \mathbf{e}_{\pi(1)} \cdots \mathbf{e}_{\pi(m)}, \mathbf{0}, \ldots, \mathbf{0} \right) \in \mathbb{R}^{m' \times m'}$ and $D = \text{diag}(\frac{c'_1}{c_1},\ldots,\frac{c'_m}{c_m}) \in \mathbb{R}^{m' \times m}$, we see that \eqref{1D} becomes 
\begin{align}
|(A - BPD)\mathbf{e}_i|_2 = |A\mathbf{e}_i - \frac{c'_i}{c_i}B\mathbf{e}_{\pi(i)}|_2 \leq \frac{\varepsilon}{|c_i|} \leq \max_{i \in [m]} |c_i|^{-1} \varepsilon \leq C'\varepsilon \ \ \text{for all } i \in [m].
\end{align}
...
We thank Fritz Sommer for turning our attention to the dictionary learning problem. We would also like to thank Darren Rhea for early explorations of this problem, and Ian Morris for posting identity \eqref{SubspaceMetricSameDim} with a reference to his proof of it on the internet (www.stackexchange.com). 
...
completed a B.S. in Physics and Chemistry at McGill University. He began his Ph.D. in Neuroscience at UC Berkeley in 2013 and joined the lab of Fritz Sommer at the Redwood Center for Theoretical Neuroscience in 2014.
