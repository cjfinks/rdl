...
\IEEEPARstart{M}{any} recent signal processing applications derive from the ability to represent each signal in a set $Y = \{ \mathbf{y}_1, \ldots, \mathbf{y}_N \} \subset \mathbb{R}^n$ as some linear superposition of only a few vectors drawn from a \emph{dictionary} $\mathcal{A} \subset \mathbb{R}^n$, where $n \leq |\mathcal{A}| \ll N$ \cite{?}. Recently, a number of algorithms have been developed to construct dictionaries for which such parsimonious decompositions of the data in $Y$ exist \cite{}. In this note, we study when these dictionaries and decompositions are unique for a given dataset and are stable with respect to perturbations of the data. These considerations are of utmost importance when the elements of such a representation are interpreted as real physical quantities to be inferred from the data.
