...
\end{align*}
%
and we introduce $\ell_k(M) := \min \left\{ \ell(M_S) : S \in {[m] \choose k} \right\}$. The reader can convince themselves that if $k' > k$ then $\ell_{k}(M) \geq \ell_{k'}(M)$ and $\ell(M) = \ell_m(M) > 0$ for any matrix $M$ with full column rank. We note that if $M$ is a real matrix then it also has bounded matrix norm $\max_{\mathbf{x} \in \mathbb{R}^m}\|M\mathbf{x}\|_2 / \|\mathbf{x}\|_2$; hence $\ell_{2k}(M) > 0$ is equivalent to $M$ having the \emph{restricted isometry property} \cite{CandesTao05} familiar from work in the field of compressed sensing.

It is also not hard to see that whenever $A$ satisfies \eqref{SparkCondition}, there is a minimal nonzero principal (or canonical) angle between all pairs of subspaces spanned by the columns of $A$ of dimensionality less than or equal to $k$. (If the largest angle between any two of these subspaces is zero, then one subspace is a subset of the other which violates the spark condition.) The smallest nonzero principal angle between a pair of subspaces, known also as the Friedrichs angle, can be defined in terms of its cosine as follows:
