...
In fact, there are many such sets of deterministically produced $\mathbf{a}_i$; we give a parametrized family in Section \ref{DUT}. Note that this theorem provides an answer to Problem \ref{DUTproblem} by the comments preceding the statement of Problem. Note also that Theorem 1 in \cite{HS11} is a direct consequence of this more general result (setting $\varepsilon = 0$), with an added improvement on $N$, the minimal number of samples required to guarantee uniqueness. 
...
To gain intuition for this quantity, if $k=2$ and $n=3$ it is the angle between the normal vectors of the two planes. We also define the following quantities for notational convenience:
...
We will not attempt to provide intuition for this quantity, other than to say it was derived in \cite{Deutsch} to describe the convergence of the alternating projections algorithm for projecting a point onto the intersection of a set of subspaces. We use it to bound the distance between a point and the interesection of a set of subspaces given an upper bound on the distance from that point to each individual subspace. 
...
The organization of the rest of this paper is as follows. In Section \ref{DUT} we state our main tool from combinatorial matrix theory and then derive from it Theorem \ref{DeterministicUniquenessTheorem}. In Section{ProbabilisticTheorems} we state probabilistic versions of Theorem \ref{DeterministicUniquenessTheorem} and give brief proofs based largely off of arguments outlined in \cite{HS11}. In Section \ref{mleqm} we describe how to extend all of these results to the case where we know only an upper bound on the number of sparse components. The final section is a discussion, and an appendix contains the proof of Lemma \ref{MainLemma}.
...
\begin{definition}
Let $V, W$ be subspaces of $\mathbb{R}^m$ and let $d(v,W) := \inf\{\|v-w\|_2: w \in W\} = \|v - \Pi_W v\|_2$ where $\Pi_W$ is the orthogonal projection operator onto subspace $W$. The \emph{gap} metric $\Theta$ on subspaces of $\mathbb{R}^{m}$ is \textbf{[see Theory of Linear Operators in a Hilbert Space p. 69 who cites first reference]}
...
We next give precise statements of our probabilistic versions of Theorem 1 and give brief proofs refering largely to the methods used in \cite{HS11}. Our statements are based upon the following construction of \emph{random sparse vectors}.
...
Fix $n, m > 0$ and  $A \in \mathbb{R}^{n \times m}$ satisfying \eqref{SparkCondition}. With probability one, $A\mathbf{a}_1, \ldots, A\mathbf{a}_k$ are linearly independent whenever the $\mathbf{a}_i$ are random $k$-sparse vectors.
\end{lemma}
...
\begin{proof}
Fix  $\sigma \in \Sigma_m$ and let $X = (\mathbf{a}_1 \cdots \mathbf{a}_N)$. By Lemma \ref{HS11lemma2}, with probability one every set of $k$ columns of $AX$ are linearly independent, i.e. $\ell_k(AX) > 0$. It follows by Theorem \ref{DeterministicUniquenessTheorem} (see the specifics outlined in Remark \ref{thm1specs}) that $\{A\mathbf{a}_1, \ldots, A\mathbf{a}_N\}$ has a stable $k$-sparse representation in $\mathbb{R}^m$ with probability one. 
\end{proof} 
...
\begin{proof}
By Lemma \ref{HS11lemma2} we have that with probability one the $\mathbf{a}_i$ are in general linear position. By the same arguments made to prove Theorem 3 in \cite{HS11}, the set of $A$ that fail to satisfy \eqref{4} form an algebraic set of measure $0$. Hence with probability one we have that $\ell_k(AX) = 0$ for $X = (\mathbf{a}_1 \cdots \mathbf{a}_N)$ only for an algebraic set of matrices $A$ of measure zero. We then apply Theorem \ref{DeterministicUniquenessTheorem} to obtain the result.
\end{proof}
