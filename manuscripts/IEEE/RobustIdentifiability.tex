...
The question of when every solution $Y = B \tilde X$ must \emph{necessarily} be of the form $\tilde X = PDX$ and $B = AD^{-1}P^{-1}$, i.e. whether $A$ and $X$ are \emph{identifiable} from $Y$ up to the ambiguity transform, was the subject of the theoretical works \cite{Georgiev05, Aharon06, Hillar15}. The following definition extends this notion of identifiability to the case when the solution $B \tilde X$ only \emph{approximates} $Y$. Let $\mathbf{e}_1, \ldots, \mathbf{e}_m$ be the standard basis in $\mathbb{R}^m$. 
