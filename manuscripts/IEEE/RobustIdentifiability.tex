...
\end{align}
%
for some permutation matrix $P \in \mathbb{R}^{m \times m}$ and invertible diagonal matrix $D \in \mathbb{R}^{m \times m}$. We say the representation is \textbf{$C$-stable} if
\begin{align}\label{Cstable}
\|A-BPD\|_2 \leq C\varepsilon
\end{align}
%
for some $C > 0$ for all $\varepsilon$ small enough. 
\end{definition}
\begin{remark}
...
We can calculate the stability constant $C$ and error threshold $\varepsilon_0$ for the $k$-sparse representation referred to in Theorem \ref{DeterministicUniquenessTheorem} as follows. Letting $X  = (\mathbf{a}_1 \cdots \mathbf{a}_N) \in \mathbb{R}^{n \times N}$ and $J(S) = \{j : \text{supp}(\mathbf{a}_j) = S\}$, define:
\begin{align}\label{epsilon0}
\varepsilon_1 := \frac{ \phi_k(A) \ell_2(A) }{\sqrt{2k^3} } \min_{S \in T} \ell_k(AX_{J(S)}).
\end{align}
If $A$ has unit norm columns, then for any matrix $B \in \mathbb{R}^{n \times m}$ and $k$-sparse $\mathbf{b}_i \in \mathbb{R}^m$ for which $\|A\mathbf{a}_i - B\mathbf{b}_i\| \leq \varepsilon$ for all $i = 1, \ldots, N$ then, provided $\varepsilon < \varepsilon_1$, we have that \eqref{Cstable} holds with
\begin{align}\label{Cdef}
C = \frac{\ell_2(A)}{\varepsilon_1} \sqrt{\frac{m}{2}}
\end{align}
%
and if $\varepsilon < \varepsilon_0$ where $\varepsilon_0 := \frac{\ell_{2k}(A)}{\sqrt{2k}C}$ then \eqref{b-PDa} holds, too.
\end{remark}
