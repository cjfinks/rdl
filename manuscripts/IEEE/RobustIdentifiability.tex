...
[**** TODO:  make all $|\cdot|$ for vectors be $|\cdot|$; let's make matrix norms be with 2 bars ****]

\IEEEPARstart{A}{} fundamental problem in data analysis is to compute representations of data that reveal intrinsic structure. In recent years, a plethora of algorithms (e.g., see \ref{} ***add punchiest recent algo refs***; review of sparse rep. \cite{Zhang2015}) have been developed for uncovering \emph{sparse} structure in datasets $Y = \{\mathbf{y}_1, \ldots, \mathbf{y}_N\} \in \mathbb{R}^n$ by approximately solving the following constrained optimization problem:
\begin{align}\label{DictionaryLearning}
\min_{B, \, |\mathbf{b}_i |_0 \leq k} \  \sum_{i=1}^N |\mathbf{y}_i - B\mathbf{b}_i|_2,
\end{align}
%
for unknown $B \in \mathbb{R}^{n \times m}$ and $\mathbf{b}_1, \ldots, \mathbf{b}_N \in \mathbb{R}^m$ with sparsity $k \ll m$ and latent dimensionality $m \ll N$. Known in the literature as ``sparse dictionary learning", ``sparse coding", or ``sparse matrix factorization", this approach emphasizes parsimony in representation by approximating each $\mathbf{y}_i$ as a linear combination of at most $k$ columns of some matrix $B$.  Typically, the optimization (\ref{DictionaryLearning}) is relaxed so that the $\ell_1$-norm $|\cdot|_1$ replaces the $\ell_0$-norm $|\cdot|_0$ (the number of nonzero entries in a vector), as the latter typically determines an NP-hard problem \cite{Tillmann2015} whereas the former allows for tractable convex programs [***Do we actually know this?  what's the latest ref on how efficient SCA is with $\ell_1$-norm***] that generically have the same solutions.

Here, we study whether solutions to \eqref{DictionaryLearning} are unique given a dataset $Y$, or at least approximately so. This is of fundamental concern for \emph{sparse component analysis} (SCA) \cite{Georgiev05}, which assumes a linear model of data:
\begin{align}\label{LinearModel}
\mathbf{y}_i = A\mathbf{a}_i + \mathbf{n}_i,
\end{align}
%
with the vector $\mathbf{n}_i \in \mathbb{R}^n$ accounting for both noise in the measurements and the degree to which the model deviates from reality (e.g., the true $\mathbf{a}_i$ may only be approximately $k$-sparse [<- we haven't defined k-sparse yet...] ). The goal in SCA is to estimate the sparse \emph{sources} $\mathbf{a}_i$ from the observed \emph{mixtures} $\mathbf{y}_i$ by approximately solving \eqref{DictionaryLearning}.

Even when all $\mathbf{n}_i = 0$, however, there are infinitely many solutions to \eqref{DictionaryLearning}. Let $Y, X_1, X_2$ be the matrices with columns $\mathbf{y}_i, \mathbf{a}_i, \mathbf{b}_i$, respectively, with $X_1, X_2$ having $k$-sparse columns.  If $Y = AX_1$, then also $Y = (AD^{-1}P^{-1})(PDX_1)$ for any permutation matrix $P \in \mathbb{R}^{m \times m}$ and invertible diagonal matrix $D \in \mathbb{R}^{m \times m}$. The ``uniqueness" of any solution to \eqref{DictionaryLearning} should therefore only be considered up to this inherent ambiguity, often called the \emph{ambiguity transform group} associated to this particular form of bilinear inverse problem. In our case, this group consists of matrices $PD$, where $P$ and $D$ are as above.  

Whether any other solution $Y = BX_2$ must necessarily be a realization of this form ($X_2 = PDX_1$ and $A = BPD$) was the subject of the theoretical works \cite{Georgiev05, Aharon06, HS15}.  
Realistically, however, empirical data is contaminated with noise, and our models are rarely (if ever) perfect. 
Therefore, we focus on the following robust formulation of SCA.
Let $\mathbf{e}_1, \ldots, \mathbf{e}_m$ be the standard basis in $\mathbb{R}^m$.

%Fix a matrix $A \in \mathbb{R}^{n \times m}$ and $k$-sparse $\mathbf{a}_1, \ldots, \mathbf{a}_N \in \mathbb{R}^m$ such that $\{A\mathbf{a}_1, \ldots, A\mathbf{a}_N\}$ has a robustly identifiable $k$-sparse representation in $\mathbb{R}^m$. Set $\delta > 0$ and suppose that $Y = \{\mathbf{y}_1, \ldots, \mathbf{y}_N\} \subset \mathbb{R}^n$ is a dataset satisfying $|\mathbf{y}_i - A\mathbf{a}_i|_2 < \varepsilon/2$ for $i = 1, \ldots, N$. By the triangle inequality, any alternate $B \in \mathbb{R}^{n \times m}$ and $k$-sparse $\mathbf{b}_1, \ldots, \mathbf{b}_N \in \mathbb{R}^m$ satisfying $|\mathbf{y}_i - B\mathbf{b}_i| < \varepsilon/2$ for $i = 1, \ldots, N$ is such that $|A\mathbf{a}_i - B\mathbf{b}_i| < \varepsilon$ for $i = 1, \ldots, N$. Hence every such alternate $B$ and corresponding $\mathbf{b}_i$ must also satisfy \eqref{def1} and \eqref{def1_}. In such cases we say the dataset $Y$ has a "unique" $k$-sparse representation in $\mathbb{R}^m$. We now state our central problem.
%Here, we focus on the motivates a robust formulation.  
...
Conditions for the identifiability of the parameters $A$ and $\mathbf{a}_i$ of the SCA model were first provided for the noiseless case ($\delta,\varepsilon = 0$) in \cite{Georgiev05} and \cite{Aharon06}, with the most general conditions recently provided by \cite{HS15}.
We refine the arguments in \cite{HS15} to account for noisy measurements, and we reduce their theoretically required number of samples (for the deterministic case) from $N=k{m \choose k}^2$ to $N = mk{m \choose k}$.  Our robust identifiability conditions describe when the true sources $\mathbf{a}_i$ and mixing matrix $A$ can be ``uniquely" determined in the sense of Definition \ref{Uniqueness}. We also provide guarantees for the case when only an upper bound on the dimensionality of the sources is known. Our conditions are derived from the underlying geometry of the dictionary learning problem, hence apply regardless of algorithm used to solve \eqref{DictionaryLearning}. 
...
Fix positive integers $n, m$, $k < m$, and a cyclic order on $\{1, \ldots, m\}$. If $k$-sparse $\mathbf{a}_1, \ldots, \mathbf{a}_N \in \mathbb{R}^m$ are such that for every interval of length $k$ in the cyclic order there are $k{m \choose k}$ vectors $\mathbf{a}_i$ in general linear position supported on that interval then every matrix $A \in \mathbb{R}^{n \times m}$ satisfying spark condition \eqref{SparkCondition} generates a set of vectors $Y = \{A\mathbf{a}_1, \ldots, A\mathbf{a}_N\}$ with a robustly identifiable $k$-sparse representation in $\mathbb{R}^m$.
...
|\mathbf{b}_i - PD\mathbf{a}_i|_2 &\leq \frac{\varepsilon }{\varepsilon_0 - \varepsilon} \left( \frac{C^{-1}+|\mathbf{a}_i|_1}{ \sqrt{2k} } \right) \ \ \text{ and }  \ \  |(A-BPD)\mathbf{e}_j|_2 \leq C\varepsilon.%, \ \ \text{ for } i = 1, \ldots, N; \, j = 1, \ldots, m. 
\end{align}
\end{theorem}

\begin{corollary}\label{DeterministicUniquenessCorollary}
Given positive integers $n, m$ and $k < m$, there exist $N =  mk{m \choose k}$ $k$-sparse vectors $\mathbf{a}_1, \ldots, \mathbf{a}_N \in \mathbb{R}^m$ with the following property: every matrix $A \in \mathbb{R}^{n \times m}$ satisfying spark condition \eqref{SparkCondition} generates a set of vectors $\{A\mathbf{a}_1, \ldots, A\mathbf{a}_N\}$ with a robustly identifiable $k$-sparse representation in $\mathbb{R}^m$.
\end{corollary}

Explicit formulae for $\varepsilon_0$ and $C$ are given in \eqref{epsilon0} and \eqref{Cdef}, below. 
It is easy to produce $\mathbf{a}_i$ with explicit values for $C,\varepsilon_0$; we give one such family in Section \ref{DUT} using ``Vandermonde" matrices. 
...
% Moreover, the generality of the construction allows us to easily extend the theorem to cases where the $A$ and $\mathbf{a}_i$ are randomly generated.
 ...
 First, we introduce some notation. We denote by $[m]$ the set $\{1, ..., m\}$ and by ${[m] \choose k}$ the set of subsets of $[m]$ of cardinality $k$. Recall that $\text{Span}\{\mathbf{v}_1, \ldots, \mathbf{v}_\ell\}$ for real vectors $\mathbf{v}_1, \ldots, \mathbf{v}_\ell$ is the vector space consisting of their $\mathbb{R}$-linear span:
%
...
The reader can convince themselves that if $k < k'$ then $\ell_{k}(M) \geq \ell_{k'}(M) > 0$ for any matrix $M$ with full column rank. We note that if $M$ is a real matrix then it also has bounded matrix norm $\max_{\mathbf{x} \in \mathbb{R}^m}|M\mathbf{x}|_2 / |\mathbf{x}|_2$; hence if $M$ is such that $\ell_{2k}(M) > 0$ then $M$ has the \emph{restricted isometry property} \cite{CandesTao05} familiar from work in the field of compressed sensing. We do not refer to the restricted isometry constant in this work since it is defined with respect to a particular scaling of the columns of the matrix, whereas we often set our matrices (without loss of generality) to have unit norm columns.
...
To gain intuition for this quantity, if $k=2$ and $n=3$ it is the angle between the normal vectors of the two planes. (In this case there is only one nonzero principal angle between non-intersecting planes.) We also define the following quantity for notational convenience:

\begin{definition}\label{SpecialSupportSet}
Fix $A \in \mathbb{R}^{n \times m}$ and $k < m$. Setting $\phi_1(A) := 1$, we define for $k \geq 2$:
\begin{align}\label{rho}
\phi_k(A) := \min_{ T \in {[m] \choose k} } \left(1 - \xi( \{\text{Span}\{A_S\} : S \in T \} ) \right),
\end{align}
%
...
\end{align}
%
and $\Sigma_p$ is the set of permutations on $p$ elements. 
\end{definition}
...
We are now in a position to explicitly state the constants $C$ and $\varepsilon_0$ referred to in Theorem \ref{DeterministicUniquenessTheorem}. Letting $X  = (\mathbf{a}_1 \cdots \mathbf{a}_N) \in \mathbb{R}^{n \times N}$ and $I(S) = \{i : \text{supp}(\mathbf{a}_i) = S\}$, define:
\begin{align}\label{epsilon0}
\varepsilon_0 = \frac{ \phi_k(A) \ell_{2k}(A) }{\sqrt{2}k^2} \min_{S \in T} \ell_k(AX_{I(S)}).
\end{align}
If $A$ has unit norm columns and $0 \leq \varepsilon < \varepsilon_0$ then \eqref{Cstable} and \eqref{b-PDa} hold with
...
\section{Deterministic Identifiability Theorem}\label{DUT}
...
We are now state our main result from combinatorial matrix theory.
...
\section{Probabilistic Identifiability Theorems}\label{PUT}
...
\end{align}
%
for some $k$-sparse $\mathbf{b}_1, \ldots, \mathbf{b}_N \in \mathbb{R}^{m'}$ then \eqref{Cstable} holds for some partial permutation matrix $P \in \mathbb{R}^{m' \times m'}$, diagonal matrix $D \in \mathbb{R}^{m' \times m}$ (i.e. $D_{ij} = 0$ whenever $i \neq j$) and $C$ as defined in \eqref{Cdef}. 
\end{theorem}
