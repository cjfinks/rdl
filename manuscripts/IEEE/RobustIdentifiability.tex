...
\IEEEPARstart{S}{parse} coding or sparse dictionary learning algorithms learn to represent each signal in a set $Y = \{\mathbf{y}_1, \ldots, \mathbf{y}_N\} \subset \mathbb{R}^n$ as a linear combination of few vectors drawn from an inferred \emph{dictionary} $\mathcal{D} \subset \mathbb{R}^n$, where $n \leq |\mathcal{D}| \ll N$ \cite{?; see \cite{} for a comphrehensive review of techniques}. One of the domains of application of these algorithms has been in solving inverse problems, wherein the dictionary elements and reconstruction coefficients are interpreted as real physical parameters to be identified from the data \cite{}. These results cannot be properly interpreted, however, without a characterization of the set of degenerate solutions and their stability with respect to noise. 

In this note, we provide conditions guaranteeing uniqueness and stability of solutions to the following formulation of the sparse coding problem. Suppose that for some $A \in \mathbb{R}^{n \times m}$ we can model $Y$ as a sequence of linear measurements:
\begin{align}\label{LinearModel}
\mathbf{y} = A\mathbf{a} + \mathbf{n}
\end{align}
%
in which the underlying \emph{sparse code} $\mathbf{a} \in \mathbb{R}^{m}$ has at most $k$ nonzero entries (\textit{$k$-sparse}) and the \emph{noise} vector $\mathbf{n}$ is bounded, $|\mathbf{n}|_2 \leq \varepsilon$. We wish to determine when solutions to the associated inverse problem:
...
\end{align}
%
are unique and vary smoothly with the level of noise. 

Phrasing the problem in this way necessitates some clarification on the term `uniqueness', however, since \eqref{InverseProblem} admits many solutions, even in the noiseless case ($\varepsilon = 0$). Let $Y$ and $X$ be matrices with columns $\mathbf{y}_i$ and $\mathbf{a}_i$, respectively, so that $Y = AX$ for $X$ with $k$-sparse columns. Then we also have $Y = BX'$ for $X'$ with $k$-sparse columns whenever $B = AD^{-1}P^{-1}$ and $X' = PDX$ with $P \in \mathbb{R}^{m \times m}$ a permutation matrix  and $D \in \mathbb{R}^{m \times m}$ an invertible diagonal. Any particular solution to \eqref{InverseProblem} thus generates many other solutions through transformations $PD$, which together constitute a \emph{transformation group} \cite{BilinInv} associated to the problem. The question of when every solution $Y = BX'$ must necessarily be of this form (i.e., $X' = PDX$ and $B = AD^{-1}P^{-1}$ for some $P$, $D$) was addressed in the theoretical works \cite{Georgiev05, Aharon06, Hillar15} and determines whether $A$ and $X$ are \emph{identifiable} from $Y$ up to this inherent ambiguity. We should note that the permutation ambiguity simply reflects on the difference between talking about a matrix and its set of columns (the dictionary), whereas the scaling ambiguity is inherent to the problem as originally stated and reflects on the difference between talking about a vector and the subspace it spans.

It remains to determine the extent to which $A$ and $X$ can be determined up to these inherent ambiguities in the presence of measurement uncertainty ($\varepsilon > 0$). To address this, we consider the following stability criterion.
...
\end{align}
known in the sparse recovery literature as the \emph{spark condition}, then the $k$-sparse representation of $Y$ in Problem \ref{DUTproblem} is robustly identifiable provided the data are sufficiently diverse. Note that this condition is necessary given only that the $\mathbf{a}_i$ are $k$-sparse. [ *** See Thm 2.8 or Cor 2.9 of identifiability of BIP paper? Can we cite this here? Can we then motivate the restrictions on X with these theorems too? ***]
