...
\begin{abstract}
It has been previously established under mild assumptions (e.g., the spark condition from compressed sensing) that the problem of inferring a dictionary matrix with respect to which a dataset admits a sparse representation has a unique solution up to inherent relabeling and scaling of the dictionary columns \cite{Hillar15}. We extend these known results to incorporate uncertainty in the generative model of the data and prove that dictionaries and the sparse representations they admit are still unique up to a commensurate error. Our results help explain the ubiquitous finding that diverse dictionary learning and sparse coding algorithms determine similar features when applied to approximately code the same sparse mixtures.  We also discuss several applications.
\end{abstract}
...
In this note, we study the uniqueness of dictionaries and sparse codes inferred from the data as solutions to the associated inverse problem:
...
An important consequence of this result is that for sufficiently small reconstruction error, the original dictionary and $k$-sparse vectors are determined up to commensurate error, independent of alternate representation. In terms of Def. \ref{Uniqueness}, for $\delta_1, \delta_2$ not too large, if \eqref{y-Bb} holds with:
...
\end{align}
%
then \eqref{def1} follows. The constant $C$ is defined in terms of $A$ and the $\mathbf{a}_i$ in \eqref{Cdef}, below. 
