...
A set of vectors $\{\mathbf{y}_1, \ldots, \mathbf{y}_N\} \subset \mathbb{R}^n$ has a \textbf{$k$-sparse representation} in $\mathbb{R}^m$ when for some $A \in \mathbb{R}^{n \times m}$ and $k$-sparse $\mathbf{a}_1, \ldots, \mathbf{a}_N \in \mathbb{R}^m$ we have $\mathbf{y}_i = A\mathbf{a}_i$ for all $i = 1, \ldots, N$. We say that $A$ and the $\mathbf{a}_i$ forming this representation are \textbf{identifiable} (up to permutation and scaling ambiguity) given $Y$ when every other $B \in \mathbb{R}^{n \times m}$ and $k$-sparse $\mathbf{b}_1, \ldots, \mathbf{b}_N \in \mathbb{R}^m$ for which $\mathbf{y}_i = B\mathbf{b}_i$ for all $i = 1, \ldots, N$ necessarily satisfy $A = BPD$ and $\mathbf{b}_i = PD\mathbf{a}_i$ for some permutation matrix $P \in \mathbb{R}^{m \times m}$ and invertible diagonal matrix $D \in \mathbb{R}^{m \times m}$. The representation is \textbf{C-stable} when there exists some $\varepsilon > 0$ such that if $B$ and the $\mathbf{b}_i$ only satisfy $\|\mathbf{y}_i - B\mathbf{b}_i\|_2 \leq \varepsilon$ for all $i = 1, \ldots, N$, then
\begin{align}\label{def1}
\|A - BPD\|_2 \leq C \max_i \|\mathbf{y}_i - B\mathbf{b}_i\|_2
\end{align}
for some $C>0$. (It follows that $\|\mathbf{b}_i - PD\mathbf{a}_i\|_2 \leq C' \max_i \|\mathbf{y}_i - B\mathbf{b}_i\|_2$ for some $C' > 0$ as well.)
\end{definition}

Suppose now that $Y = \{\mathbf{y}_1, \ldots, \mathbf{y}_N\} \subset \mathbb{R}^n$ is a dataset for which for some $A \in \mathbb{R}^{n \times m}$ and $k$-sparse $\mathbf{a}_1, \ldots, \mathbf{a}_N \in \mathbb{R}^m$ we have $\|\mathbf{y}_i - A\mathbf{a}_i\|_2 \leq \varepsilon$ for $i = 1, \ldots, N$ for some $\varepsilon > 0$. That is, every data point can be approximated (up to error $\varepsilon$) by some vector $A\mathbf{a}_i$ lying in a subspace spanned by $k$ columns of $A$. If the set $\{A\mathbf{a}_1, \ldots, A\mathbf{a}_N\}$ has a $C$-stable $k$-sparse representation in $\mathbb{R}^m$ then by the triangle inequality, provided $\varepsilon$ small enough, any alternate $B \in \mathbb{R}^{n \times m}$ and $k$-sparse $\mathbf{b}_1, \ldots, \mathbf{b}_N \in \mathbb{R}^m$ satisfying $\|\mathbf{y}_i - B\mathbf{b}_i\| \leq \varepsilon$ for $i = 1, \ldots, N$ must also satisfy \eqref{def1}. We say such datasets $Y$ have $C$-stable $k$-sparse \emph{approximate representations} in $\mathbb{R}^m$. That is, a dataset has a $C$-stable $k$-sparse approximate representation when the data can be approximated well enough by a set of vectors with a $C$-stable $k$-sparse representation.

Some readers may find the following alternative definition more appealing:

\begin{definition}\label{Uniqueness}
We say a dataset $Y = \{\mathbf{y}_1, \ldots, \mathbf{y}_N\} \subset \mathbb{R}^n$ has a $C$-stable $k$-sparse approximation in $\mathbb{R}^m$ when:
\begin{enumerate}
...
\end{align}
%
for some $\sigma > 0$. If $A$ has unit norm columns and satisfies spark condition \eqref{SparkCondition}, then for any matrix $B \in \mathbb{R}^{n \times m}$ and $k$-sparse $\mathbf{b}_i \in \mathbb{R}^m$ for which $\|\mathbf{y}_i - B\mathbf{b}_i\| < \frac{\alpha_{2k}(A)^2\sigma\phi_k(A)}{(2k)^{2/3}}$ for all $i = 1, \ldots, N$ we have
\begin{align}
\|A - BPD\|_2 \leq \left( \frac{2k^{2/3}}{\alpha_k(A)\sigma \phi_k(A)} \right) \max_i \|\mathbf{y}_i - B\mathbf{b}_i\|
\end{align}
%
...
\|\mathbf{b}_i - PD\mathbf{a}_i\|_1 \leq \frac{\max_i \|\mathbf{y}_i - B\mathbf{b}_i\|}{\alpha_{2k}(A)\|D\|_2}  \left( 1 + \frac{2 k^{2/3}}{\alpha_k(A)\sigma \phi_k(A)}\|D^{-1}P^{-1}\mathbf{b}_i\|_1 \right).
\end{align}
\end{remark}
...
Fix positive integers $n, m$ and $k$ such that $k < m$. Let $A, B \in \mathbb{R}^{n \times m}$ and suppose that $A$ satisfies the spark condition \eqref{SparkCondition} with unit norm columns. Let $\mathcal{T} = \{S_0, \ldots, S_{m-1}\}$. If there exists a map $\pi: \mathcal{T} \to {[m] \choose k}$ and some $\Delta < \frac{\alpha_{2k}(A)}{\sqrt{2}}$ such that 
...
In this section we prove a stronger version of Lemma \ref{MainLemma} by assuming that $B$ also satisfies the spark condition. In doing so, we can address the issue of recovering $A \in \mathbb{R}^{n \times m}$ and the $\mathbf{a}_i \in \mathbb{R}^m$ when only an upper bound $m'$ on the number $m$ of sparse sources is known.

\begin{theorem}\label{DeterministicUniquenessTheorem2}
Given positive integers $n, m, m'$ and $k$ with $k < m \leq m'$, there exist $N =  mk{m' \choose k}$ $k$-sparse vectors $\mathbf{a}_1, \ldots, \mathbf{a}_N \in \mathbb{R}^m$ with the following property: if $Y = \{ \mathbf{y}_1, ..., \mathbf{y}_N \}$ is a dataset for which for some $A \in \mathbb{R}^{n \times m}$ satisfying spark condition \eqref{SparkCondition} we have $\| \mathbf{y}_i - A\mathbf{a}_i \|_2 \leq \varepsilon$ for all $i = 1, \ldots, N$, then any other matrix $B \in \mathbb{R}^{n \times m'}$ also satisfying spark condition \eqref{SparkCondition} and for which there exist $k$-sparse $\mathbf{b}_1, \ldots, \mathbf{b}_N \in \mathbb{R}^{m'}$ such that $\| \mathbf{y}_i - B\mathbf{b}_i \|_2 \leq \varepsilon$ for all $i = 1, \ldots, N$ necessarily satisfies $\|A - BPD\| \leq C\varepsilon$ for some $C > 0$, partial permutation matrix $P \in \mathbb{R}^{m' \times m'}$ and diagonal matrix $D \in \mathbb{R}^{m' \times m}$, provided $\varepsilon$ is small enough. Moreover, this implies $\|\mathbf{b}_i - PD\mathbf{a}_i\|_1 \leq C'\varepsilon$ for some $C' > 0$.

Specifically, the $\mathbf{a}_i$ are constructed so that any $k$ of them are linearly independent. In particular, any set of $k$ vectors $\mathbf{a}_i$ that share the same support satisfy \eqref{DataSpread} for some $\sigma > 0$. Letting $\Phi := \min(\phi_k(A), \phi_k(B))$ and letting $\alpha_k := \min( \alpha_k(A), \alpha_k(B) )$ and $\alpha_{2k} := \min( \alpha_{2k}(A), \alpha_{2k}(B) )$, then provided $\varepsilon < (2k)^{-2/3}\alpha_k^2\sigma \Phi$ we have
\begin{align}
...
\caption{$B$, $PD$, $\hat{A}$: left-to-right noises $0, .2, .4, 1, 2, 5$. *** Need to replace with correct latest figures (e.g. with PD taken care of properly).***}
\end{center}
\end{figure*}
...
\begin{proof} 
Fix $x \in \mathbb{R}^m$ and $k \geq 2$. We will do the proof in two steps. First, we will show that
...
We now show that there exists no additional $i \notin \{v - (k-1), \ldots, v\}$ such that $p \in \pi(S_{i})$. This would complete the proof by induction as follows: by letting $Q_{m-1} \subset Q_m$ be the set of elements of $Q_m$ not having $p$ as a second coordinate, we would have $|Q_{m-1}| = (m-1)k$ and the proof would then follow by iterating the arguments above. Note that if there were such an $i$ then we would have $p \in \pi(S_{i}) \cap \pi(S_{v - (k-1)}) \cap \cdots \cap \pi(S_{v})$ and \eqref{EmptyToEmpty} would imply that every $k$-element subset of $\{i\} \cup \{v-(k-1), \ldots, v\}$ is a set of consecutive integers. This is only possible if $m = k+1$; but then there cannot have been $k+1$ distinct elements of ${\mathbb{Z}/m\mathbb{Z} \choose k}$ all containing $p$ since there are only ${m-1 \choose m-2}  = m-1 = k$ distinct elements of ${\mathbb{Z}/m\mathbb{Z} \choose m-1}$ which contain $p$. 
\end{proof}
...
d\left( \mathbf{z}, \cap_{i \in \mathcal{I}} \text{Span}\{A_{S_i}\} \right) \leq \Delta \left( \frac{\phi_k(A)}{1 - c(\text{Span}\{A_{S_{i_1}}\}, \ldots, \text{Span}\{A_{S_{i_\ell}}\})} \right) \leq \Delta.
\end{align}
%
...
\section{Assuming the Spark Condition on $B$}
