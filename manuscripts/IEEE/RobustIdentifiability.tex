...
[*** Just fucking around with a possible alternate intro ***] 
...
\end{align}
\end{theorem}
[****** Put $|\mathbf{b}_i - PD\mathbf{a}_i|_1$ formulation instead *******]
...
We are now in a position to explicitly state the constants $C$ and $\varepsilon_0$ referred to in Theorem \ref{DeterministicUniquenessTheorem}. Letting $X  = (\mathbf{a}_1 \cdots \mathbf{a}_N) \in \mathbb{R}^{n \times N}$, $I(S) := \{i : \text{supp}(\mathbf{a}_i) = S\}$, and $T$ be the set of intervals of length $k$ in a cyclic order of $[m]$, set:
...
Before proving Theorem \ref{DeterministicUniquenessTheorem} in full generality, consider the case when $k=1$. Here, we have $N = m$ and we set $\mathbf{a}_1, \ldots, \mathbf{a}_N$ to be the standard basis vectors $\mathbf{e}_i \in \mathbb{R}^m$ $(i = 1, \ldots, m)$. Fix some $A \in \mathbb{R}^{n \times m}$ satisfying spark condition \eqref{SparkCondition} with unit norm columns. Note that \eqref{epsilon0} yields $\varepsilon _0 = \ell_2(A)/\sqrt{2}$ in this case and that $\ell_2(A) \leq 1$. Suppose that for some $B \in \mathbb{R}^{n \times m}$ and 1-sparse $\mathbf{b}_i \in \mathbb{R}^m$ we have  $|A\mathbf{a}_i - B\mathbf{b}_i|_2 \leq \varepsilon < \varepsilon_0$ for all $i \in [m]$. Since the $\mathbf{b}_i$ are 1-sparse, there must exist $c_1, \ldots, c_m \in \mathbb{R}$ such that 
\begin{align}\label{1D}
|A\mathbf{e}_i - c_iB\mathbf{e}_{\pi(i)}|_2 \leq \varepsilon, \ \ \text{for all} \  i \in [m],
\end{align}
and some map $\pi: [m] \to [m]$. 
Note that if $c_i = 0$ for some $i$ then $|A\mathbf{e}_i|_2 < 1/\sqrt{2}$ which contradicts that $A$ has unit norm columns. We will now show that $\pi$ is necessarily injective (and thus defines a permutation). Suppose that $\pi(i) = \pi(j) = \ell$ for some $i \neq j$ and $\ell \in [m]$. Then, $|A\mathbf{e}_i - c_iB\mathbf{e}_{\ell}|_2  \leq \varepsilon$ and $|A\mathbf{e}_j - c_jB\mathbf{e}_{\ell}|_2 \leq \varepsilon$. Summing and scaling these two inequalities by $|c_j|$ and $|c_i|$, respectively, we have:
\begin{align*}
(|c_i| + |c_j|) \varepsilon
...
\end{align*}
%
which is in contradiction with the fact that $|x|_1 \leq \sqrt{2}|x|_2$ for all $x \in \mathbb{R}^2$ and $\varepsilon < \frac{\ell_2(A)}{\sqrt{2}}$. Hence, $\pi$ is injective. Letting $P = \left( \mathbf{e}_{\pi(1)} \cdots \mathbf{e}_{\pi(m)}\right)$ and $D = \text{diag}(c_1,\ldots,c_m)$, 
...
% ======== b - PDa =========
From this result we can, in general, bound $|\mathbf{b}_i - PD\mathbf{a}_i|_2$ as well. Specifically, we will show that \eqref{b-PDa} always follows from \eqref{Cstable} when $\varepsilon < \varepsilon_0$, with $\varepsilon_0$ as defined in \eqref{epsilon0} and $C$ is defined as in \eqref{Cdef}. Note that for all $2k$-sparse $\mathbf{x} \in \mathbb{R}^m$ we have by the triangle inequality:
\begin{align}
|(A-BPD)\mathbf{x}|_2 
\leq |(A-BPD)\mathbf{e}_i|_2 |\mathbf{x}|_1 
