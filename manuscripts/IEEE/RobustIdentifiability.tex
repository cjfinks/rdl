...
\IEEEPARstart{A}{} number of modern approaches to signal processing model datasets $Y = \{\mathbf{y}_1, \ldots, \mathbf{y}_N\} \subset \mathbb{R}^n$ with unknown structure as lying near a collection of low-dimensional subspaces spanned by the columns $\{A_1,\ldots,A_m\}$ of a learned \textit{dictionary} matrix $A \in \mathbb R^{n\times m}$.  Specifically, data vectors $\mathbf{y} \in Y$ are modeled as arising from a linear \emph{measurement}:
\begin{align}\label{LinearModel}
\mathbf{y} = A\mathbf{a} + \mathbf{n},
\end{align}
in which the underlying \emph{source} $\mathbf{a} \in \mathbb{R}^{m}$ has at most $k$ nonzero entries (\textit{$k$-sparse}) and the \emph{noise} vector $\mathbf{n}$ has bounded energy, $|\mathbf{n}|_2 \leq \varepsilon$. 
...
In this note, we consider the well-posedness of the associated inverse problem: 
...
It is easy to see how a solution to Problem \ref{DUTproblem} directly informs the interpretation of any solution to \eqref{InverseProblem}. Let $\delta_1, \delta_2 > 0$ and set $\varepsilon = \frac{1}{2}\varepsilon(\delta_1, \delta_2)$, where $\varepsilon(\delta_1, \delta_2)$ is as in Def.~\ref{Uniqueness}. From \eqref{LinearModel}, we have $|\mathbf{y}_i - A\mathbf{a}_i|_2 \leq \varepsilon$ for all $i$; thus, by the triangle inequality, any solution to \eqref{InverseProblem} necessarily satisfies \eqref{def1} for some $P$ and $D$.

The main finding of this work is that if the matrix $A \in \mathbb{R}^{n \times m}$ is injective on the set of $k$-sparse vectors: 
...
\end{align}
known in the literature as the \emph{spark condition}, then the $k$-sparse representation of $Y$ in Problem \ref{DUTproblem} is robustly identifiable provided the data are sufficiently diverse. Note that this condition is necessary given only that the $\mathbf{a}_i$ are $k$-sparse. 
...
An important consequence of this result is that for sufficiently small reconstruction error, the original dictionary and sparse vectors are determined up to commensurate error, independent of alternate representation. In other words, if 
%$\varepsilon = \varepsilon(\delta_1, \delta_2)$, where
\begin{align}\label{epsd1d2}
\varepsilon(\delta_1, \delta_2) =  \min \left( \delta_1 C^{-1}, \frac{ \delta_2 \varepsilon_0 + \varepsilon \min_{i \in [N]} |\mathbf{a}_i|_1}{C^{-1} + \delta_2}\right),
\end{align}
then  the robust identifiability criteria of Def. \ref{Uniqueness} is satisfied. The constant $C$ is defined in terms of $A$ and the $\mathbf{a}_i$ in \eqref{Cdef}, below. 
...
[ *** We should look over this again...since we're not mentioning L0 minimization in the intro anymore *** ]
While certain algorithms have been shown to have local convergance to a global minimum or global convergence to a local minimum of approximations to \eqref{InverseProblem}, to our knowledge there exist no algorithms with guaranteed global convergence to a global solution. Our conditions apply regardless of the algorithm used to solve \eqref{InverseProblem} since they are derived from the underlying geometry of the dictionary learning problem.  In particular, we provide for a universal check when any dictionary learning algorithm has converged to (near) a global minimum of \eqref{InverseProblem}.
...
Given that a robust version of uniqueness in SCA is possible, it would be useful to determine the best dependence of $\varepsilon$ on $\delta_1, \delta_2$ (see Def. \ref{Uniqueness}) as possible as well as the minimum possible sample size $N$.  In the case of $k = 1$, the best possible bounds are given by [********  CHAZ?? ***********].  We encourage the sparse coding community to extend our results and find a tight dependency of all the parameters, both for the sake of theory and practical applications.

Another field of computation for which our main results have consequences is that of smoothed analysis \cite{spielman2004smoothed}.  The main idea there is that certain algorithms having bad worst case behavior, nonetheless, are efficient if certain (typically Lebesgue measure zero) pathological input sets are avoided.  In the case of SCA, our results imply that if there is an efficient ``smoothed" algorithm, then only for a measure zero set of inputs will this algorithm fail to determine the unique original solution.  We note that avoiding ``bad" (NP-hard) sets of inputs is necessary in the case of SCA \cite{Tillmann15}.

\textbf{Signal Processing and Data Analysis}.
In applications of machine learning to data science, and specifically the application of SCA, it is common to interpret learned features as intrinsic structure in the data.  However, as there are a diverse array of algorithms employed to compute such features, it is unclear whether such learned representations of data are inherent in the data or artifacts of the SCA method.  Here, our results suggest that uniqueness in SCA for data analysis is the norm rather than the exception.  In particular, the robust uniqueness proved here gives justification for declaring that unique features of the data (e.g. receptive fields of neurons  \cite{agarwal2014spatially}) have been found.

\textbf{Brain Communication Theory}.
In \cite{coulter2010adaptive, isely2010deciphering}, it was posited that sparse features of natural data could be passed through a communication bottleneck in the brain using random projections and SCA.  A necessary condition for this theory to work is that there is a uniqueness of SCA.  This was verified in the case of data sampled without noise in \cite{Hillar15}.  The present work extends this theory to the more realistic case of sampling noise.
...
We thank Darren Rhea and Fritz Sommer for useful comments, and Ian Morris for posting identity \eqref{SubspaceMetricSameDim} with a reference to his proof of it on the internet (``Stack Exchange").
