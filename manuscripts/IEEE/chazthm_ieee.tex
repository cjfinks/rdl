...
The proof is very similar to the proof of Thm.~\ref{DeterministicUniquenessTheorem} in Sec.~\ref{DUT}, the difference being that now we establish a map $\pi: [m] \to [m']$ satisfying the requirements of Lem.~\ref{MainLemma2} by pigeonholing $(k-1){m' \choose k} + 1$ vectors with respect to holes $[m']$. The value of $C$ in this case is:
...
The same manipulations in Rem.~\ref{b-PDaProof} show how \eqref{b-PDa} then follows from \eqref{Cstable} for some subset of $m$ coefficients in the $\mathbf{b}_i$ as well. 
