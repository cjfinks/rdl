...
% FRITZ2: don't say sparse coding model. just say in sparse coding. Also, dictionary learning is the term to use for this crowd.
\section{Introduction}\label{Intro}
\IEEEPARstart{T}{he} process of optimizing a dictionary for sparse coding first emerged as a derivation to explain the response properties of neurons in mammalian primary visual cortex \cite{Olshausen96, hurri1996image, bell1997independent, van1998independent} and has since demonstrated its power as a tool for solving a variety of problems in signal processing and machine learning (see \cite{Zhang15} for a comprehensive review). In this framework, each vector in a dataset $Z = \{\mathbf{z}_1, \ldots, \mathbf{z}_N\} \subset \mathbb{R}^n$ is encoded by a linear combination of at most $k$ vectors drawn from an inferred dictionary $\mathcal{A} \subset \mathbb{R}^n$ of size $m$, where $k < m \ll N$. 
...
% seems not too bad
Many dictionary learning algorithms have been designed to discover such ``sparse structure" in data. Unfortunately, it remains unclear in general when one can expect these inferences to be consistent. 
%In certain empirical studies it seems the learned structure generalizes well to unseen data.
For instance, sparse coding of local painting style with learned dictionaries has been proposed as a tool for forgery detection \cite{hughes2010, Olshausen10}. It could be, however, that several different dictionaries are consistent with training data while differing in their ability to sparsely encode new samples. %from the same distribution. 
...
The main finding of this work is that any dictionary satisfying the spark condition \eqref{SparkCondition} from compressed sensing (CS) is uniquely identifiable from enough generic noisy sparse linear combinations of its elements up to an error linear in the noise (Thm.~\ref{DeterministicUniquenessTheorem}). In fact, provided $(n, m, k)$ satisfy the nearly-optimal CS inequality \eqref{CScondition}, in almost all cases the dictionary learning problem is well-posed in the sense of Hadamard \cite{Hadamard1902} (Cor.~\ref{ProbabilisticCor}). These guarantees can also be extended to the case when only an upper bound on $m$ is known (Thm.~\ref{DeterministicUniquenessTheorem2}). 

We hope the explicit, algorithm-independent criteria contained herein will serve as theoretical tools in the analysis of sparse coding routines, some of which now provably converge to a unique solution under certain conditions (see \cite[Sec.~I-E]{Sun16} for a brief summary of the state-of-the-art). 
...
The problem of learning an optimal dictionary for sparse coding can be stated as follows. Fix a dictionary represented as the columns $A_j$ of a matrix $A \in \mathbb R^{n \times m}$ and suppose $Z$ consists of measurements:
