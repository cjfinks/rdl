...
As an important consequence, Thm.~\ref{DeterministicUniquenessTheorem} guarantees that \eqref{def1} holds provided $\varepsilon$ is smaller than:
\begin{align*}
...
Hence, $\delta < \rho$. Since $\phi_k \leq 1$, we also have $d(\mathbf{u}, \text{Span}\{B_{\pi(S_j)}\}) < 1$, and it follows by Lem.~\ref{MinDimLemma} that $\dim(\text{Span}\{B_{\pi(S_j)}\}) \geq \dim(\text{Span}\{A_{S_j}\}) = k$. Since $|\pi(S_j)| = k$, we in fact have $\dim(\text{Span}\{B_{\pi(S_j)}\}) = k$. %, i.e. the columns of $B_{\pi(S_\sigma(i))}$ are linearly independent. 
