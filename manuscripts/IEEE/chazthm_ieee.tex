...
Consider the symbolic dataset $Y = \{A\mathbf{a}_1,\ldots,A \mathbf{a}_N\}$, where the supports of the indeterminate $k$-sparse vectors $\mathbf{a}_i$ are consecutive length-$k$ intervals in some cyclic order on $[m]$. Those substitutions of real numbers for the indeterminates denying $Y$ a robust sparse coding has Borel measure zero.
\end{lemma}
\begin{proof}
\end{proof}
