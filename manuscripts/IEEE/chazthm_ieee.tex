...
\title{When is dictionary learning for sparse coding a well-posed problem?}
...
\begin{abstract}
Dictionary learning methods for sparse coding have exposed underlying structure in many kinds of natural signals.  However, universal theorems guaranteeing the statistical consistency of inference in this model are lacking.  Here, we prove that for almost all diverse enough datasets consistent with this model, the latent dictionary and sparse codes are uniquely identifiable up to an error commensurate with measurement noise. Applications are given to data analysis, neuroscience, and engineering.
\end{abstract}
...
% seems not too bad
Certain applications demand a better understanding of when data admit a unique such `sparse structure'. For instance, detecting forgeries by analysis of local painting style \cite{hughes2010, Olshausen10} requires that all dictionaries consistent with training data do not differ appreciably in their ability to sparsely encode new samples. Recently, algorithms with provable convergence guarantees under certain conditions have been proposed  (see \cite[Sec.~I-E]{Sun16} for a brief summary of the state-of-the-art). It remains unknown, however, when the problem is well-posed in general (as per Hadamard \cite{Hadamard1902}), independent of any algorithm.
...
where $\mathcal{B} = \{ \mathbf{x} \in \mathbb{R}^n: |\mathbf{x}|_2 \leq 1\}$.\footnote{Note that $\cos\theta(U,V) < 1$ for all subspaces $U,V \subseteq \mathbb{R}^m$.}
...
Fix $A \in \mathbb{R}^{n \times m}$ and $k < m$ and let $T \subseteq 2^{[m] \choose k}$ contain at least $k$ elements. For $k > 1$, define:
\begin{align*}
...
\end{align*} 
%
and set $\phi_T(A) := 1$ for $k=1$.
\end{definition}
Note that we always have $\phi_T(A) > 0$, since $\xi(\mathcal{V}) < 1$ always. (Otherwise, we would have $\cos \theta(U,V) = 1$ for some subspaces $U,V \subseteq \mathbb{R}^m$, which is easily shown to lead to a contradiction.)%, in contradiction with $(U \cap V) \cap (U \cap V)^\perp = \emptyset$.

%=== SPECIFICS OF DETERMINISTIC THEOREM ===%
We are now in a position to state the constant $C > 0$ referred to in Thm. ~\ref{DeterministicUniquenessTheorem}:
...
\begin{lemma}\label{NonEmptyLemma} Fix integers $k < m$, and let $T = \{S_1, \ldots, S_m\}$ be the set of intervals of length $k$ in some cyclic order of $[m]$. Suppose there exists a map $\pi: T \to {[m] \choose k}$ such that:
...
We assume $k \geq 2$ since the case $k = 1$ was proven in Sec.~\ref{DUT}. We assume w.l.o.g. that the cyclic order on $[m]$ is simply $[m]$ and let $T = \{S_1, \ldots, S_m\}$ so that $S_i = \{i, \ldots, i+k-1\}$ for $i \in [m]$ are the intervals of length $k$ in the order. We begin by showing that $\dim(\text{Span}\{B_{\pi(S)}\}) = k$ for all $S \in T$. 
...
Hence, $\delta < \rho$. Since $\phi_T(A) \leq 1$, we also have $d(\mathbf{u}, \text{Span}\{B_{\pi(S)}\}) < 1$, and it follows by Lem.~\ref{MinDimLemma} that $\dim(\text{Span}\{B_{\pi(S)}\}) \geq \dim(\text{Span}\{A_{S}\}) = k$. Since $|\pi(S)| = k$, we in fact have $\dim(\text{Span}\{B_{\pi(S)}\}) = k$, i.e. the columns of $B_{\pi(S)}$ are linearly independent. 
...
\end{align*}
%
where the second inequality follows from the Def.~\ref{SpecialSupportSet}. Recall that $\xi < 1$ always.
% TODO: 
...
C= \left( \frac{ \sqrt{k^3}}{ \min(\phi_T(A), \phi_{\pi[T]}(B)) } \right) \frac{\max_{j \in [m]} |A_j|_2}{\min_{S \in T} L_k(AX_{I(S)})}.
\end{align}
