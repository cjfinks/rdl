...
\begin{align}
h(A) = \prod_{S \in {[m] \choose k}} \sum_{R \in {[n] \choose k}} |\det(A_{(R,S)})|
\end{align}
%
where for any $R \in {[n] \choose k}$ and $S \in {[m] \choose k}$, the symbol $A_{(S,R)}$ denotes the submatrix of entries $A_{ij}$ with $(i,j) \in R \times S$.  
