...
% Some source had said: these angles are all zero if and only if one subspace is a subset of the other; otherwise, $\theta$ is the smallest nonzero such angle \cite{?}.} %\cite[Def.~9.4]{Deutsch12}.
\end{definition}
...
%[** Wait, hold up - this statement contradicts Lemma 9.5 (8) in Deutsch which says the *cosine* is zero if the subspaces are subsets of one another, in which case the angle is $\pi/2$. **]
...
Fix $A \in \mathbb{R}^{n \times m}$ and $k < m$. Setting $\phi_1(A) := 1$, define for $k > 1$:
\begin{align*}
...
\begin{align*}
\xi(\mathcal{V}) := \min_{\sigma \in \frak{S}_k} \left(1 -  \prod_{i=1}^{k-1} \sin^2  \theta \left(V_{\sigma(i)}, \cap_{j=i+1}^k V_{\sigma(j)} \right)  \right)^{1/2},
\end{align*}
and $\frak{S}_{k}$ are the permutations (bijections) on $k$ elements. 
\end{definition}
% TODO: Error! If we set all the supports to be equal then phi_k(A) = 0. They must be distinct. In the proof of Lemma 1 they are all intervals in a cyclic order. I think this only needs to be taken over the cyclic order for which the a_i are supported.
% cos\theta = 0 if and only if Pv and Pu commute -> sin\theta = 1 if and only if Pv and Pu commute
...
\end{align}
%
where $T$ is the set intervals of length $k$ in a cyclic order on $[m]$, $X$ is the $m \times N$ matrix with columns $\mathbf{a}_i$, and $I(S) := \{i : \text{supp}(\mathbf{a}_i) = S\}$.\footnote{We can be sure that $C > 0$ is well-defined. First, $|A_j|_2 > 0$ for all $j$ by \eqref{SparkCondition}. Next, $\min_{S \in T} L_k(AX_{I(S)}) > 0$ by the fact that $L_k(A) > 0$ and every $k$ columns of $X_{I(S)}$ are linearly independent. Finally, $\phi_k(A) = 0$ implies ???}
...
\end{lemma}
\begin{proof} 
Fix $\mathbf{x} \in \mathbb{R}^m$ and $k \geq 2$. Recall that the orthogonal projection onto a closed subspace $V \subseteq \mathbb{R}^m$ is the mapping $\Pi_V: \mathbb{R}^m \rightarrow V$ which associates with each $\mathbf{x} \in \mathbb{R}^m$ its unique nearest point in $V$:
\begin{align*}
|\mathbf{x} - \Pi_V\mathbf{x}|_2 = d(\mathbf{x}, V) := \inf\{|\mathbf{u}-\mathbf{v}|_2: \mathbf{v} \in V\}.
\end{align*}

The proof consists of two parts. First, we shall show that: 
...
% TODO: sum over the same indices as in the definition of \xi?
...
\end{align}
for \mbox{$z^2= 1 - \prod_{\ell =1}^{k-1}(1-z_{\ell}^2)$} and \mbox{$z_{\ell} = \cos\theta\left(V_{\ell}, \cap_{s=\ell+1}^k V_s\right)$}. 
%for \mbox{$z = \left[ 1 - \prod_{\ell =1}^{k-1}\left(1-\cos^2\theta_F\left(V_{\ell}, \cap_{s=\ell+1}^k V_s\right)\right) \right]^\frac{1}{2}$}. 
...
\end{align}
Finally, since $z$ might vary with the (arbitrary) ordering of the subspaces $V_i$, we substitute $\xi(\mathcal{V})$ for $z$ in \eqref{ceq} to obtain \eqref{DTILeq}.
\end{proof}
...
Fix $S \in T$ and note that by \eqref{GapUpperBound}, all unit vectors $\mathbf{u} \in \text{Span}\{A_{S}\}$ satisfy $d(\mathbf{u}, \text{Span}\{B_{\pi(S)}\}) \leq \frac{\phi_k(A)}{\rho k} \delta$ for $\delta < \frac{L_2(A)}{ \sqrt{2}}$. By definition of $L_2(A)$, for all $2$-sparse $\mathbf{x} \in \mathbb{R}^m$:
\begin{align*}
L_2(A) \leq \frac{|A\mathbf{x}|_2}{|\mathbf{x}|_2} \leq \rho \frac{|\mathbf{x}|_1}{|\mathbf{x}|_2} \leq \rho \sqrt{2}.
\end{align*}

Hence, $\delta < \rho$. Since $\phi_k(A) \leq 1$, we also have $d(\mathbf{u}, \text{Span}\{B_{\pi(S)}\}) < 1$, and it follows by Lem.~\ref{MinDimLemma} that $\dim(\text{Span}\{B_{\pi(S)}\}) \geq \dim(\text{Span}\{A_{S}\}) = k$. Since $|\pi(S)| = k$, we in fact have $\dim(\text{Span}\{B_{\pi(S)}\}) = k$. %, i.e. the columns of $B_{\pi(S_\sigma(i))}$ are linearly independent. 
...
\end{align*}
%
where the 2nd inequality follows from the definition of $\phi_k(A)$. We can be sure in the above that $\xi < 1$, as required by Lem.~\ref{DistanceToIntersectionLemma}, since ** TODO **.
% TODO: 
%1) Show that $\xi < 1$. Suppose $\xi = 1$. Then for all ordering of the arguments V_i, we have sin^2 \theta (V_i, intersect of j > i) = 0 for some i, or equivalently cos\theta(V_i, intersect of j > i) = 1 for some i. 
