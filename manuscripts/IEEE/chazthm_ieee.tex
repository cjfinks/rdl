...
It so happens that a similar statement applies to sets of vectors that have a stable sparse representation. As in \cite{Hillar15} (Sec.~IV), consider the ``symbolic" set $Y = \{A\mathbf{a}_1,\ldots,A \mathbf{a}_N\}$ generated by indeterminate $A$ and $k$-sparse $\mathbf{a}_1, \ldots, \mathbf{a}_N$. 
