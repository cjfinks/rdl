...
\IEEEPARstart{T}{he} emergence of response properties of neurons in the mammalian visual cortex from algorithms designed to optimize dictionaries for sparse coding of natural images marked an exciting development in computational neuroscience \cite{Olshausen96, hurri1996image, bell1997independent, van1998independent}. Many dictionary learning algorithms have been developed since and applied to a variety of problems in signal processing and machine learning (see \cite{Zhang15} for a comprehensive review). A popular formulation of the idea is to encode each of $N$ data points as a linear combination of at most $k$ $n$-dimensional vectors from an inferred dictionary of size $m$, where $k < m \ll N$. 
