...
\end{align}
known in the sparse recovery literature as the \emph{spark condition}, then the $k$-sparse representation of $Y$ in Problem \ref{DUTproblem} is stable provided the data are sufficiently diverse. Note that this condition is necessary given only that $\mathbf{x}_i$ are $k$-sparse. [ *** TODO: Right? Can we motivate more necessary conditions using theorems of Li15, too? Theorem 2.8. *** ] 
...
\begin{remark}[Support Recovery]
If the difference between two $m$-dimensional vectors has a small enough $\ell_1$-norm and one of them is $k$-sparse for small enough $k$ then its support in fact contains the support of the other. Hence for small enough $\varepsilon$, the composition [*** TODO: I think this is right, finish this, see Donoho paper on L1 stability***]
...
We state here the main implications of our probabilistic results. To keep exposition simple, our statements are based upon the following elementary construction of \emph{random sparse vectors} (although many ensembles suffice, e.g. \cite[Sec.~\S 4]{Baraniuk08}).
