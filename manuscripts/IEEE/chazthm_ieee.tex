...
% TODO: Chris are you on the right grant here?
\maketitle

\begin{abstract}
Sparse coding has exposed underlying structure in many kinds of natural data.  However, given the multitude of algorithms implementing this strategy, claims of ``true" latent discovery require the backing of universal theorems guaranteeing statistical consistency.  Here, we prove that for almost all diverse enough datasets generated under this model, sparse coding uniquely identifies the original dictionary and codes up to an error commensurate with measurement noise.  Applications are given to smoothed analysis, neuroscience, and engineering.
\end{abstract}
...
\end{align}
for $k$-\emph{sparse} $\mathbf{a} \in \mathbb{R}^m$ having at most $k$ nonzero entries and \emph{noise} $\mathbf{n} \in \mathbb{R}^n$ with $\ell_2$-norm $|\mathbf{n}|_2$. The noise represents our combined worst-case uncertainty in  measuring $A\mathbf{a}$.
...
Note that any particular solution to this problem represents a whole class of equivalent solutions $B = AD^{-1}P^{\top}$ and $\mathbf{b}_i = PD\mathbf{a}_i$, where $P \in \mathbb{R}^{m \times m}$ is any permutation matrix and $D \in \mathbb{R}^{m \times m}$ any invertible diagonal matrix, but that we are concerned with establishing the uniqueness of this class and not the individual solutions. Previous works \cite{li2004analysis, Georgiev05, Aharon06, Hillar15} addressing the noiseless case $\varepsilon = 0$ have proved that the solutions (when they exist) are indeed unique in this sense for sufficiently large $N$ provided the matrix $A$ satisfies the \textit{spark condition}:
...
which is evidently a necessary uniqueness condition if the $\mathbf{a}_i$ are known only to be $k$-sparse. Matrices of the form $PD$ thus form the \emph{ambiguity transformation group} inherent to the noiseless problem when $N$ is large enough \cite{Li15}. We introduce the following terminology to handle the noisy case $\varepsilon > 0$.
...
When does $Y \subset \mathbb{R}^n$ have a stable $k$-sparse representation in $\mathbb{R}^m$?
\end{question}

To see how a positive answer to this question informs the interpretation of solutions to Problem \ref{InverseProblem}, suppose that \mbox{$Y = \{A \mathbf{a}_1, \ldots, A\mathbf{a}_N\}$} has a stable $k$-sparse representation in $\mathbb R^m$ and fix $\delta_1, \delta_2$ to be the desired accuracy in recovery \eqref{def1}.  Consider any dataset $Z$ generated as in \eqref{LinearModel} with each $|\mathbf{n}|_2 \leq \frac{1}{2} \varepsilon(\delta_1, \delta_2)$.  Then, any  dictionary $B \in \mathbb{R}^{n \times m}$ and $k$-sparse $\mathbf{b}_1, \ldots, \mathbf{b}_N \in \mathbb{R}^m$ solving Problem \ref{InverseProblem} with $\varepsilon \leq \frac{1}{2} \varepsilon(\delta_1, \delta_2)$ are identical to the originals, up to the desired accuracy \eqref{def1}.  

Our results, listed in Sec.~\ref{Results}, include an explicit form for $\varepsilon(\delta_1, \delta_2)$. We prove Thm.~\ref{DeterministicUniquenessTheorem} in Sec.~\ref{DUT} after listing some additional definitions and key lemmas required for the proof, including our main tool from combinatorial matrix analysis (Lem.~\ref{MainLemma}). Our proof is a refinement of the arguments in \cite{Hillar15} to handle noise and to reduce the number of required samples from $N=k{m \choose k}^2$ to $N = m(k-1){m \choose k}+m$. All other proofs are relegated to the supplementary materials. The final section contains a discussion of potential applications.
...
In this note, we generalized recent results \cite{Hillar15} on the uniqueness of solutions to Problem~\ref{InverseProblem} in the noiseless case to the case of noisy measurements, while also reducing the number of required samples from $N=k{m \choose k}^2$ to $N = m(k-1){m \choose k}+m$. Surprisingly, almost all $n \times m$ dictionaries satisfying the standard assumption \eqref{CScondition} from compressed sensing (CS) are identifiable from $N$ generic noisy $k$-sparse linear combinations of their elements, up to an error linear in the noise. Moreover, if solutions are constrained to satisfy \eqref{SparkCondition} then only an upper bound on the size of $\mathcal{A}$ need be known a priori. We note furthermore that all of these results extend trivially to the case where certain point-wise injective nonlinearities are applied to the data. We close by outlining  four diverse application areas.
...
Our results provide theoretical grounding for the application of sparse coding to inverse problems (``sparse component analysis"), wherein the linear model \eqref{LinearModel} is assumed to describe some truth about the data and the goal is to infer the generating dictionary and sparse codes from noisy measurements. Regarding this, it would be useful for practical applications to determine for general $(m,n,k)$ the best possible dependence of $\varepsilon$ on $\delta_1, \delta_2$ (see Def. \ref{Uniqueness}) as well as the minimal requirements on the number and diversity of generating codes. We encourage researchers to extend our results and find tight dependencies on all parameters.
