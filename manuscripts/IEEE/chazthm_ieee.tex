...
Fix $n, m$, $k < m$ and suppose that for every interval of length $k$ in some cyclic order on $[m]$ at least \mbox{$(k-1){m \choose k}+1$} of the vectors $\mathbf{a}_i$ are supported (symbolically) on that interval. There is a polynomial in the entries of $A$ and the $\mathbf{a}_i$ with the property that when real numbers are substituted for the indeterminates, if the polynomial evaluates to a nonzero number then $Y$ has a stable $k$-sparse representation in $\mathbb{R}^m$. In particular, if $(m,n,k)$ allow for a single such substitution to impart to $Y$ this property, then almost all such substitutions (up to a Borel measure zero set) do.
\end{theorem}
...
In this note, we generalize recent results \cite{Hillar15} on the uniqueness of solutions to Problem~\ref{InverseProblem} in the noiseless case to the case of noisy measurements and reduce the number of required samples from $N=k{m \choose k}^2$ to $N = m(k-1){m \choose k}+m$. Surprisingly, almost all $n \times m$ dictionaries satisfying the standard assumption \eqref{CScondition} from compressed sensing (CS) are uniquely identifiable up to an error linear in the noise from enough generic $k$-sparse noisy linear combinations of their elements. Moreover, only an upper bound on the size of $\mathcal{A}$ need be known a priori if solutions are constrained to satisfy \eqref{SparkCondition}. We note furthermore that all of these results extend trivially to the case where certain point-wise injective nonlinearities are applied to the data. We close by outlining  four diverse application areas.
...
Applications of sparse coding to inverse problems assume that \eqref{LinearModel} describes some true model of the data and the goal is to infer the true underlying dictionary and sparse codes from the noisy measurements. Our results provide theoretical grounding for why a variety of sparse coding routines often succeed at recovering true underlying parameters of this model \cite{??}. Regarding this, it would be useful to determine for general $(m,n,k)$ the best possible dependence of $\varepsilon$ on $\delta_1, \delta_2$ (see Def. \ref{Uniqueness}) as well as the minimum possible sample size $N$ and ``diversity" of generating codes. We encourage researchers to extend our results and find tight dependencies on all parameters, both for theory and practical applications.
