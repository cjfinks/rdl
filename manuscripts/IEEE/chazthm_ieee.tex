...
Fix $n, m$, $k < m$, and matrix \mbox{$A = (A_{ij})_{i,j=1}^{n,m}$} of indeterminates. 
Consider the symbolic dataset \mbox{$Y = \{A\mathbf{a}_1,\ldots,A \mathbf{a}_N\}$}, where the supports of the indeterminate $k$-sparse vectors satisfy: 
for every interval of length $k$ in some cyclic order on $[m]$ there are at least \mbox{$(k-1){m \choose k}+1$} vectors $\mathbf{a}_i$ with that (symbolic) support. There is a polynomial $g$ having the property that when it is nonzero, the resulting $Y$ has a stable sparse representation in $\mathbb R^m$.
In particular, if $(m,n,k)$ allow a single dataset with a stable representation, then almost all (up to a Borel measure zero set) of them do.
\end{theorem}
...
This polynomial was produced in \cite[Sec.~IV]{Hillar15}.
\end{proof}
