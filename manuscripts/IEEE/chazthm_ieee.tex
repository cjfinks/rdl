...
Let $A \in \mathbb{R}^{n \times m}$ and the $\mathbf{a}_i \in \mathbb{R}^m$ be as described in Thm.~\ref{DeterministicUniquenessTheorem} and fix $m' > m$. If $B \in \mathbb{R}^{n \times m'}$ also satisfies the spark condition and \mbox{$|A\mathbf{a}_i - B\mathbf{b}_i|_2 \leq \varepsilon$} for all $i \in [N]$ for some set of $k$-sparse $\mathbf{b}_i \in \mathbb{R}^{m'}$, then there exists a constant $C > 0$ such that \eqref{Cstable} and \eqref{b-PDa} hold with respect to a \textbf{partial} permutation matrix $P \in \mathbb{R}^{m' \times m'}$ (i.e., there is at most one nonzero entry in each row and column and these nonzero entries are all one) and a diagonal matrix $D \in \mathbb{R}^{m' \times m}$ (e.g. $D_{ij} = 0$ when $i \neq j$). 
\end{theorem}
