...
Any solution to this problem admits an inherent permutation-scaling ambiguity, since $B = AD^{-1}P^{\top}$ and set of $k$-sparse $\mathbf{b}_i = PD\mathbf{a}_i$ also give the same reconstruction error for any permutation matrix $P \in \mathbb{R}^{m \times m}$ and invertible diagonal matrix $D \in \mathbb{R}^{m \times m}$. Previous works \cite{li2004analysis, Georgiev05, Aharon06, Hillar15}  addressing the case $\varepsilon = 0$ have proved that the solutions are indeed unique up to this inherent ambiguity for sufficiently large $N$ provided the matrix $A$ satisfies the \textit{spark condition}:
