...
%\IEEEPARstart{A}{n} exciting development in computational neuroscience when the first algorithms to optimize dictionaries for sparse coding were applied to natural images and reproduced response properties of neurons in mammalian visual cortex \cite{Olshausen96, hurri1996image, bell1997independent, van1998independent}.
%\IEEEPARstart{A}{n} exciting development in computational neuroscience was the reproduction of response properties of neurons in the mammalian visual cortex by the first algorithms to optimize dictionaries for sparse coding \cite{Olshausen96, hurri1996image, bell1997independent, van1998independent}.
\IEEEPARstart{A}{n} exciting discovery in computational neuroscience was that the response properties of neurons in the mammalian visual cortex could be reproduced by algorithms designed to optimize dictionaries for sparse coding  of natural images \cite{Olshausen96, hurri1996image, bell1997independent, van1998independent}. Since then, many dictionary learning algorithms have been developed and applied to a variety of problems in signal processing and machine learning (see \cite{Zhang15} for a comprehensive review). A popular formulation of the idea is to encode each datum as a linear combination of at most $k$ $n$-dimensional vectors from an inferred dictionary of size $m$, where $k < m \ll N$. 
...
which is evidently a necessary condition given that the $\mathbf{a}_i$ are known only to be $k$-sparse.\footnote{For example, no such uniqueness exists when $A$ has rank 1.} %TODO: Make PERFECT
...
\[ \delta = \frac{ \rho k}{ \phi_\mathcal{T}(A) } \left(  \frac{\varepsilon \sqrt{k}}{L(AX_{J(S)})} \right) = C\varepsilon < L_2(A)/\sqrt{2}. \]
