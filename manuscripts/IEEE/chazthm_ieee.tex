...
\title{When is sparse coding via dictionary learning well-posed?}
...
\begin{abstract}
Dictionary learning for sparse coding has exposed underlying structure in many kinds of natural signals.  However, universal theorems guaranteeing the statistical consistency of inference in this model are lacking.  Here, we prove that for almost all diverse enough datasets consistent with this model, the latent dictionary and sparse codes are uniquely identifiable up to an error commensurate with measurement noise. Applications are given to data analysis, neuroscience, and engineering.
\end{abstract}
...
%\IEEEPARstart{E}{ver} since sparse coding emerged as a compelling explanation for the response properties of neurons in mammalian primary visual cortex \cite{Olshausen96, hurri1996image, bell1997independent, van1998independent}, the process of optimizing a dictionary for sparse representation of vector-valued data has become an important step in many signal processing and machine learning applications (see \cite{Zhang15} for a comprehensive review). In the sparse coding via dictionary learning framework, each point in a dataset $Z = \{\mathbf{z}_1, \ldots, \mathbf{z}_N\} \subset \mathbb{R}^n$ is coded as a linear combination of at most $k$ vectors drawn from a learned dictionary $\mathcal{A} \subset \mathbb{R}^n$, where $k < |\mathcal{A}| \ll N$. 
\IEEEPARstart{A}{n} exciting development in neuroscience took place when the first algorithms to optimize dictionaries for sparse coding were applied to natural images and reproduced response properties of neurons in mammalian visual cortex \cite{Olshausen96, hurri1996image, bell1997independent, van1998independent}. Since then, many so-called `dictionary learning' algorithms have been developed and applied to a variety of problems in signal processing and machine learning (see \cite{Zhang15} for a comprehensive review). A popular approach is to encode each vector in a dataset $Z = \{\mathbf{z}_1, \ldots, \mathbf{z}_N\} \subset \mathbb{R}^n$ as a linear combination of at most $k$ vectors drawn from an inferred dictionary $\mathcal{A} \subset \mathbb{R}^n$ of size $m$, where $k < m \ll N$ \cite{Aharon06}. 
%\IEEEPARstart{E}{ver} since sparse coding of natural images with an optimized dictionary reproduced response properties of neurons in mammalian primary visual cortex \cite{Olshausen96, hurri1996image, bell1997independent, van1998independent}, a plethora of dictionary learning algorithms have been developed and applied to a variety of problems in signal processing and machine learning (see \cite{Zhang15} for a comprehensive review). In this model, each vector in a dataset $Z = \{\mathbf{z}_1, \ldots, \mathbf{z}_N\} \subset \mathbb{R}^n$ is encoded by a linear combination of at most $k$ vectors drawn from an inferred dictionary $\mathcal{A} \subset \mathbb{R}^n$ of size $m$, where $k < m \ll N$. 
...
% FRITZ2: Painting is confusing. It's not unique, but rather unique to each painter -- still, it could be that each painter has a number of dictionaries associated to them.
%TODO: Is Olshausen10 reference really appropriate?
% seems not too bad
Certain applications may require an understanding of when a dataset admits a unique such `sparse structure'. For instance, the detection of forgeries by analysis of local painting style \cite{hughes2010, Olshausen10} assumes that all dictionaries consistent with training data do not differ appreciably in their ability to sparsely encode new samples. Recently, algorithms with provable convergence to a unique solution under certain conditions have been proposed  (see \cite[Sec.~I-E]{Sun16} for a brief summary of the state-of-the-art). It remains unknown, however, when this problem is well-posed in general (as per Hadamard \cite{Hadamard1902}), independent of algorithm.
...
The main finding of this work is that any dictionary satisfying the spark condition \eqref{SparkCondition} from compressed sensing (CS) is uniquely identifiable from enough generic noisy sparse linear combinations of its elements up to an error linear in the noise (Thm.~\ref{DeterministicUniquenessTheorem}). In fact, provided $(n, m, k)$ satisfy the nearly-optimal CS inequality \eqref{CScondition}, in almost all cases the dictionary learning problem is well-posed (Cor.~\ref{ProbabilisticCor}). These guarantees are easily extended to the case when only an upper bound on $m$ is known (Thm.~\ref{DeterministicUniquenessTheorem2}). We hope the explicit, algorithm-independent criteria contained herein may serve as a theoretical tool in future analyses of dictionary learning procedures.
...
We state the dictionary learning problem considered here more precisely as follows. Fix a dictionary represented as the columns $A_j$ of a matrix $A \in \mathbb R^{n \times m}$ and suppose $Z$ consists of measurements:
...
Note that any particular solution to this problem in fact represents a whole class of equivalent solutions $BPD$ and $D^{-1}P^{\top}\mathbf{b}_i$, where $P \in \mathbb{R}^{m \times m}$ is any permutation matrix and $D \in \mathbb{R}^{m \times m}$ any invertible diagonal matrix. Since such arbitrary scalings and ordering of dictionary elements represent the same underlying model, it is natural to ask whether solutions to Problem~\ref{InverseProblem} are unique only up to this equivalence.
...
The \textbf{angle} between subspaces $U,V \subseteq \mathbb{R}^n$ is the angle $\theta(U,V)$ in $(0,\frac{\pi}{2}]$ whose cosine is given by \cite[Def.~9.4]{Deutsch12}:
\begin{align*}
...
where $\mathcal{B} = \{ \mathbf{x} \in \mathbb{R}^n: |\mathbf{x}|_2 \leq 1\}$.%\footnote{ $\theta$ is one out of a set of \textit{principal} (or \textit{canonical} or \textit{Jordan}) angles between subspaces that are invariant to orthogonal transformations \cite{?}.} 
% Some source had said: these angles are all zero if and only if one subspace is a subset of the other; otherwise, $\theta$ is the smallest nonzero such angle \cite{?}.} But this contradicts the definition, since theta is pi/2 when one subspace is contained in the other.%\cite[Def.~9.4]{Deutsch12}.
\end{definition}
...
Fix $A \in \mathbb{R}^{n \times m}$, fix a cyclic order on $[m]$, and let $T$ denote the set of contiguous intervals of length $k$ in the order for some $k < m$. For $k > 1$, define:
\begin{align*}
%\phi_k(A) := \min_{ S_1 \neq \ldots \neq S_k \in {[m] \choose k} } 1 - \xi( A_{S_1}, \ldots, A_{S_k}),
\phi_T(A) :=  \min_{R \in {T \choose k}} 1 - \xi( \{ A_{S}: S \in R\}),
\end{align*}
...
\end{align*} 
%
and $\phi_T(A) := 1$ for $k=1$. 
\end{definition}
Note that we always have $\phi_T(A) > 0$, since $\xi(\mathcal{V}) = 1$ implies that $\cos \theta(U,V) = 1$ for some pair of subspaces $U,V \in \mathbb{R}^m$, in contradiction with $(U \cap V) \cap (U \cap V)^\perp = \emptyset$.
% TODO: Does contiguous really convey shifting by 1? Or does it convey actual side by side intervals?

%=== SPECIFICS OF DETERMINISTIC THEOREM ===%
The constant $C > 0$ referred to in Thm. ~\ref{DeterministicUniquenessTheorem} is then:
\begin{align}\label{Cdef}
C = \left( \frac{ \sqrt{k^3}}{ \phi_T(A) } \right) \frac{\max_{j \in [m]} |A_j|_2}{\min_{S \in T} L_k(AX_{I(S)})},
\end{align}
%
where $T$ is the set intervals of length $k$ in a cyclic order on $[m]$, $X$ is the $m \times N$ matrix with columns $\mathbf{a}_i$, and $I(S) := \{i : \text{supp}(\mathbf{a}_i) = S\}$.\footnote{We can be sure that $C$ is positive and well-defined, since $|A_j|_2 > 0$ for all $j$ by \eqref{SparkCondition} and $\min_{S \in T} L_k(AX_{I(S)}) > 0$ by the fact that $L_k(A) > 0$ and every $k$ columns of $X_{I(S)}$ are linearly independent.}
...
The \textbf{gap} (or \textbf{aperture}) $\Theta(U,V)$ between subspaces $U, V \subseteq \mathbb{R}^m$ is given by \cite{Kato p.197}: %\cite{Akhiezer13}:
\begin{equation*}
...
\Theta(A_{S}, B_{\pi(S)}) \leq \frac{ \phi_T(A) }{\rho k} \delta, \indent \text{for all } S \in T,
\end{equation}
%
...
&\leq \frac{\phi_T(A) L_2(A)}{\rho k \sqrt{2}}.
\end{align}
Since $L_2(A) \leq \rho \sqrt{2}$ and $\phi_T(A) \leq 1$, the RHS of \eqref{rhs} is strictly less than one. It follows by Lem.~\ref{MinDimLemma} that $\dim(\text{Span}\{B_{S'}\}) \geq \dim(\text{Span}\{A_{S}\}) = k$. % (since every $k$ columns of $A$ are linearly independent). 
...
Thus, from \eqref{rhs} and \eqref{yeyeye} it follows that, for any particular $S \in T$, the inequality in \eqref{GapUpperBound} is satisfied for $\delta < L_2(A)/\sqrt{2}$ by setting $\delta = \frac{ \rho k}{ \phi_T(A) } \left(  \frac{\varepsilon \sqrt{k}}{L(AX_{J(S)})} \right)$ (see Rem.~\ref{nonzero} for why \mbox{$\phi_T(A) > 0$}). Hence, \eqref{GapUpperBound} holds (for all $S \in T$) when $\delta = C\varepsilon$. %, with $C$ as defined in \eqref{Cdef}.
%\begin{align*}
...
Our results provide theoretical grounding for the use of sparse coding for blind source separation, wherein the goal is to infer the generating dictionary and sparse codes (e.g., representing a rat's position on a linear track \cite{Agarwal14}) from noisy measurements (e.g., local field potentials in Hippocampus) generated as in \eqref{LinearModel}. To this end, it would be useful to determine the best possible dependence of $\varepsilon$ on $\delta_1, \delta_2$ (see Def. \ref{Uniqueness}) as well as the minimal requirements on the number and diversity of generating codes. We encourage researchers to extend our results and tighten these parameters.

\textbf{Theoretical Neuroscience}.
Sparse coding and related methods have repeatedly been shown to recover characteristic sparse components of natural images \cite{Olshausen96, hurri1996image, bell1997independent, van1998independent}, sounds \cite{bellsejnowski1996, smithlewicki2006, Carlson12} and smells \cite{}, reproducing response properties of some cortical neurons. Together with these results, our theorems suggest that the sparse representations of these signals may indeed, in some sense, be unique. Furthermore, these theoretical guarantees justify the hypothesis of \cite{Coulter10} and \cite{Isely10} that sparse representations of natural data passed through a communication bottleneck in the brain could be recovered from random projections via (unsupervised) biologically plausible sparse-coding algorithms (e.g., \cite{rozell2007neurally, hu2014hebbian}).   %TODO: cite Joel Z paper? Arora "simple, efficient, and neural algos for sparse coding"?
...
\end{lemma}
\begin{proof} 
Fix $\mathbf{x} \in \mathbb{R}^m$ and $k \geq 2$. Recall that the orthogonal projection onto a subspace $V \subseteq \mathbb{R}^m$ is the mapping $\Pi_V: \mathbb{R}^m \rightarrow V$ which associates with each $\mathbf{x} \in \mathbb{R}^m$ its unique nearest point in $V$:
\begin{align*}
...
\end{align*}
%
by the triangle inequality and the fact that $\|\Pi_{V_{\ell}}\|_2 \leq 1$ for all $\ell$ (as $\Pi_{V_{\ell}}$ are projections).
...
\end{align}
for \mbox{$z^2= 1 - \prod_{\ell =1}^{k-1}(1-z_{\ell}^2)$} and \mbox{$z_{\ell} = \cos\theta\left(V_{\ell}, \cap_{s=\ell+1}^k V_s\right)$}. Together with the fact that $\Pi_{V_\ell} \Pi_V = \Pi_V$ for all $\ell = 1, \ldots, k$ and $\Pi_V^2 = \Pi_V$, this yields:
\begin{align*}
...
\begin{lemma}\label{NonEmptyLemma} Fix integers $k < m$, and let $T = \{S_1, \ldots, S_m\}$ be the set of contiguous intervals of length $k$ in some cyclic order of $[m]$. Suppose there exists a map $\pi: T \to {[m] \choose k}$ such that:
...
We assume $k \geq 2$ since the case $k = 1$ was proven in Sec.~\ref{DUT}. We assume w.l.o.g. that the cyclic order on $[m]$ is simply $[m]$ and let $T = \{S_1, \ldots, S_m\}$ so that $S_i = \{i, \ldots, i+k-1\}$ are contiguous intervals of length $k$ in the order. We begin by showing that $\dim(\text{Span}\{B_{\pi(S)}\}) = k$ for all $S \in T$. 
Fix $S \in T$ and note that by \eqref{GapUpperBound}, all unit vectors $\mathbf{u} \in \text{Span}\{A_{S}\}$ satisfy $d(\mathbf{u}, \text{Span}\{B_{\pi(S)}\}) \leq \frac{\phi_T(A)}{\rho k} \delta$ for $\delta < \frac{L_2(A)}{ \sqrt{2}}$. By definition of $L_2(A)$, for all $2$-sparse $\mathbf{x} \in \mathbb{R}^m$:
\begin{align*}
L_2(A) \leq \frac{|A\mathbf{x}|_2}{|\mathbf{x}|_2} \leq \rho \frac{|\mathbf{x}|_1}{|\mathbf{x}|_2} \leq \rho \sqrt{2}.
\end{align*}

Hence, $\delta < \rho$. Since $\phi_T(A) \leq 1$, we also have $d(\mathbf{u}, \text{Span}\{B_{\pi(S)}\}) < 1$, and it follows by Lem.~\ref{MinDimLemma} that $\dim(\text{Span}\{B_{\pi(S)}\}) \geq \dim(\text{Span}\{A_{S}\}) = k$. Since $|\pi(S)| = k$, we in fact have $\dim(\text{Span}\{B_{\pi(S)}\}) = k$. %, i.e. the columns of $B_{\pi(S_\sigma(i))}$ are linearly independent. 
...
Fix $J \in {[m] \choose k}$. By \eqref{GapUpperBound}, for all unit vectors \mbox{$\mathbf{u} \in \cap_{j \in J} \text{Span}\{B_{\pi(S_j)}\}$}, we have  that $d(\mathbf{u}, A_{S_j}) \leq \frac{\phi_T(A)}{\rho k} \delta$ for all $j \in J$, where $\delta < \frac{L_2(A)}{\sqrt{2}}$. It follows from Lem.~\ref{DistanceToIntersectionLemma} that:
\begin{align*}
d\left( \mathbf{u}, \bigcap_{j \in J} \text{Span}\{A_{S_j}\} \right) 
\leq \frac{\delta}{\rho} \left( \frac{ \phi_T(A) }{1 - \xi( \{ A_{S_j}: j \in J\} ) } \right) \leq \frac{\delta}{\rho},
\end{align*}
%
where the second inequality follows from the Def.~\ref{SpecialSupportSet}. We can be sure in the above that $\xi < 1$, as required by Lem.~\ref{DistanceToIntersectionLemma}, since $\phi_T(A) > 0$ always.
% TODO: 
...
\Theta(A_{S}, B_{\pi(S)}) \leq \frac{ \delta }{\rho k} \min(\phi_T(A), \phi_T(B)),
\end{equation}
...
C= \left( \frac{ \sqrt{k^3}}{ \min(\phi_T(A), \phi_T(B)) } \right) \frac{\max_{j \in [m]} |A_j|_2}{\min_{S \in T} L_k(AX_{I(S)})}.
\end{align}
