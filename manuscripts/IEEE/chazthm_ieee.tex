...
\title{When is dictionary learning for sparse coding well-posed?}
...
\begin{abstract}
Methods for optimizing dictionaries for sparse coding expose underlying structure in many kinds of natural signals.  However, universal theorems guaranteeing the statistical consistency of inference in this model are lacking.  Here, we prove that for almost all diverse enough datasets consistent with this model, the latent dictionary and sparse codes are uniquely identifiable up to an error commensurate with measurement noise. Applications are given to data analysis, neuroscience, and engineering.
\end{abstract}
...
The main finding of this work is that any dictionary satisfying the spark condition \eqref{SparkCondition} from compressed sensing (CS) is identifiable from $N = m + m(k-1){m \choose k}$ generic noisy sparse linear combinations of its elements up to an error linear in the noise (Thm.~\ref{DeterministicUniquenessTheorem}). In fact, provided $(n, m, k)$ satisfy the nearly-optimal CS inequality \eqref{CScondition}, in almost all cases the dictionary learning problem is well-posed given enough data (Cor.~\ref{ProbabilisticCor}). These guarantees extend easily to the case when only an upper bound on $m$ is known (Thm.~\ref{DeterministicUniquenessTheorem2}). We hope the explicit, algorithm-independent criteria which we provide may serve as a useful tool in analyses of dictionary learning procedures.
...
where $\mathcal{B} = \{ \mathbf{x} \in \mathbb{R}^n: |\mathbf{x}|_2 \leq 1\}$.\footnote{It is easy to check that $\cos\theta(U,V) < 1$ for all subspaces $U,V \subseteq \mathbb{R}^m$.}
...
\end{align*} 
%
and $\phi_T(A) := 1$ for $k=1$.\footnote{Note that we always have $\phi_T(A) > 0$, since $\xi(\mathcal{V}) < 1$ always; otherwise, we would have $\cos \theta(U,V) = 1$ for some subspaces $U,V \subseteq \mathbb{R}^m$.}%, in contradiction with $(U \cap V) \cap (U \cap V)^\perp = \emptyset$.
\end{definition}
...
Sparse coding and related methods have repeatedly been shown to recover characteristic sparse components of natural images \cite{Olshausen96, hurri1996image, bell1997independent, van1998independent}, sounds \cite{bellsejnowski1996, smithlewicki2006, Carlson12} and smells \cite{}, reproducing response properties of some cortical neurons. Together with these results, our theorems suggest that the sparse representations of these signals may indeed, in some sense, be unique. Furthermore, our guarantees justify the hypothesis of \cite{Coulter10} and \cite{Isely10} that sparse codes passed through a communication bottleneck in the brain could be recovered from random projections via (unsupervised) biologically plausible sparse-coding algorithms (e.g., \cite{rozell2007neurally, hu2014hebbian}).   
%TODO: cite Joel Z paper? Arora "simple, efficient, and neural algos for sparse coding"?
%TODO: cite olfaction sparse coding
...
%TODO: Check T is right to be used for both A and B here...
